...
/**
 * Http client to 'register' a single use key. This is the process of sending a single-use key and a
 * new api key password to the backend. The backend will 'consume' the single-use key, mark it as
 * used, generate a new key salted with the password provided and will then pass the new key to the
 * front-end. Note, the backend does not store the new key value, it hashes it with the password.
 * Through this one-time process, once the key leaves the backend server, only the front-end client
 * will know the key value.
 */
...
public class ToolboxRegisterNewKeyClient {
  ...
  public RegisterApiKeyResult registerNewKey(final ApiKeyPassword apiKeyPassword) {
    ...
    try {
      ...
      return RegisterApiKeyResult.builder().errorMessage(e.getMessage()).build();
    }
  }
}
