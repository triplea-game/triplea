 ...
 * as a floating point number to be decoded. Code snippet to do this is inspired by:
 * https://stackoverflow.com/questions/22310143/java-8-localdatetime-deserialized-using-gson
 * <p>
 * The expected representation that we will be parsing is: {@code [epoch second].[epoch nano]}.
  ...
  static Gson decoder() {
    return new GsonBuilder().registerTypeAdapter(
        ...
        (JsonDeserializer<Instant>) (json, type, jsonDeserializationContext) -> {
          Preconditions.checkState(json.getAsJsonPrimitive().getAsString().contains("."),
              "Unexpected json date format, expected {[epoch second].[epoch nano]}, "
                  + "value received was: " + json.getAsJsonPrimitive().getAsString());
  ...
  private static long[] splitTimestamp(final JsonElement json) {
    ...
    Preconditions.checkState(split.length == 2,
        "Unexpected JSON {[epoch second].[epoch nano]} timestamp format, value was: "
            + json.getAsJsonPrimitive().getAsString() + ", and was split into: " + Arrays.asList(split));
