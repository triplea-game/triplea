...
class ToolboxRegisterNewKeyClientTest {
  ...
  private static final ApiKeyPassword API_KEY_PASSWORD = ApiKeyPassword.builder()
  ...
  void registerNewKey(@WiremockResolver.Wiremock final WireMockServer server) {
    ...
    final RegisterApiKeyResult result =
        newClient(server).registerNewKey(API_KEY_PASSWORD);
    ...
    server.stubFor(
        WireMock.post(ToolboxRegisterNewKeyClient.REGISTER_API_KEY_PATH)
            ...
            .willReturn(WireMock.aResponse()
                .withStatus(status)
                .withBody(
                    HttpClientTesting.toJson(REGISTER_API_KEY_RESULT))));
  }
