...
class MapsListingClientTest extends WireMockTest {
  ...
  private static MapsListingClient newClient(final WireMockServer wireMockServer) {
    return newClient(wireMockServer, MapsListingHttpClient::new);
  }

  @Test
  void sendGameHostingRequest(@WiremockResolver.Wiremock final WireMockServer wireMockServer) {
    wireMockServer.stubFor(
        get(MapsListingClient.MAPS_LISTING_PATH)
            .willReturn(
                WireMock.aResponse()
                    .withStatus(200)
