...
/**
 * DAO used to lookup single-use keys, insert single-use keys, and insert (permanent) moderator
 * api-keys.
 */
public interface ModeratorApiKeyDao {
  ...
  @SqlUpdate(
      "update moderator_api_key\n"
          + "set date_last_used = now(), last_used_host_address = :usedByHostAddress\n"
          + "where api_key = :apiKey")
  int recordKeyUsage(
      @Bind("apiKey") String cryptedValue, @Bind("usedByHostAddress") String usedByHostAddress);
