...
public interface ErrorReportingDao {
  ...
  /**
   * Counts how many records, how many error reports, a user has submitted since a given date.
   *
   * @param userIp Ip host address of the user submitting an error report.
   * @param dateSince How far back to look, only records newer compared to this date will be
   *     counted.
   */
  @SqlQuery(
      "select count(*) from error_report_history "
          + "where user_ip = :ip "
          + "  and date_created > :dateSince")
  int countRecordsByIpSince(@Bind("ip") String userIp, @Bind("dateSince") Instant dateSince);
