...
final class ErrorReportingDaoTest {
  ...
  void insertionsSince() {
    ...
    final Instant farPast = LocalDateTime.of(2000, 1, 1, 0, 0).toInstant(ZoneOffset.UTC);
    ...
    MatcherAssert.assertThat(
        ...
        REPORTING_DAO.countRecordsByIpSince(
            "first", LocalDateTime.of(2016, 1, 3, 23, 0, 0).toInstant(ZoneOffset.UTC)),
        Is.is(1));

    MatcherAssert.assertThat(
        "should select last two records of first userIp",
        REPORTING_DAO.countRecordsByIpSince(
            "first", LocalDateTime.of(2016, 1, 2, 23, 0, 0).toInstant(ZoneOffset.UTC)),
        Is.is(2));

    MatcherAssert.assertThat(
        "should select all three records of first userIp",
        REPORTING_DAO.countRecordsByIpSince(
            "first", LocalDateTime.of(2016, 1, 1, 23, 0, 0).toInstant(ZoneOffset.UTC)),
        Is.is(3));
  }
