...
public class MapRouteDrawer {
  ...
  private final RouteCalculator routeOptimizer;
  private final MapData mapData;
  private final MapPanel mapPanel;

  public MapRouteDrawer(final MapPanel mapPanel, final MapData mapData) {
    routeOptimizer = new RouteCalculator(mapData.scrollWrapX(), mapData.scrollWrapY(), mapPanel.getImageWidth(),
  ...
  private void drawCurvedPath(final Graphics2D graphics, final Point[] points, final int xOffset, final int yOffset,
    ...
    List<Line> lines = routeOptimizer.getAllNormalizedLines(xcoords, ycoords);
    for (Line line : lines) {
