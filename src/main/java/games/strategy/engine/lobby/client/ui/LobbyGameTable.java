...
class LobbyGameTable extends JTable {
  ...
  public Component prepareRenderer(final TableCellRenderer renderer, final int rowIndex, final int colIndex) {
    ...
    if (this.getModel() instanceof LobbyGameTableModel) {
      final LobbyGameTableModel lmodel = (LobbyGameTableModel) this.getModel();
      final GameDescription gd = lmodel.get(rowIndex);
