...
public class MacFinder
{
    public static String GetMacAddress()
    {
        ...
        try
        {
            ...
            if (localHostNI != null)
            {
                byte[] mac = localHostNI.getHardwareAddress();
                String macString = convertMacBytesToString(mac);
                if (isMacValid(macString))
                    return macString;
            }
        }
        catch (Exception ex)
        ...
        try
        {
            ...
            if (results != null && results.trim().length() > 0)
            {
                ...
                String getmacMac = results.substring(macStartIndex, 17);
                if (getmacMac != null)
                {
                    String macString = getmacMac.replace("-", ".");
                    if(isMacValid(macString))
                        return macString;
                }
            }
