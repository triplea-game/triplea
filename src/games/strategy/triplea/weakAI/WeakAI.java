    ...
    protected void purchase(boolean purcahseForBid, int PUsToSpend, IPurchaseDelegate purchaseDelegate, GameData data, PlayerID player)
    {
        if (purcahseForBid)
        {
            ...
            while(minCost == Integer.MAX_VALUE ||  leftToSpend >= minCost)
            {
                for(ProductionRule rule : rules)
                {
                    ...
                    if(Matches.UnitTypeIsSea.match(results) || Matches.UnitTypeIsAir.match(results) ||  Matches.UnitTypeIsAAOrIsFactoryOrIsInfrastructure.match(results) || Matches.UnitTypeHasMaxBuildRestrictions.match(results) || Matches.UnitTypeConsumesUnitsOnCreation.match(results))
                    {
                        continue;
                    }
        ...
        while(minCost == Integer.MAX_VALUE ||  leftToSpend >= minCost)
        {
            for(ProductionRule rule : rules)
            {
                ...
                UnitType results = (UnitType) rule.getResults().keySet().iterator().next();
                if(Matches.UnitTypeIsAir.match(results) ||  Matches.UnitTypeIsAAOrIsFactoryOrIsInfrastructure.match(results) || Matches.UnitTypeHasMaxBuildRestrictions.match(results) || Matches.UnitTypeConsumesUnitsOnCreation.match(results))
                {
                    continue;
                }
