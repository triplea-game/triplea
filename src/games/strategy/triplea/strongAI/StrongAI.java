		...
		for (Territory t : transTerr2)
		{
			...
			while (tTIter.hasNext() && !mytrans.isEmpty())
			{
				...
				if (seaRoute == null) //This is half of the fix for the transport issue where AI doesn't move amphibiously in some situations, such as America on Great War
                                {
                                    Match friendlyWater = new CompositeMatchAnd<Territory>(Matches.TerritoryIsWater, Matches.territoryHasNoEnemyUnits(player, data));
			            seaRoute = data.getMap().getRouteWithEndCondIgnored_CutEarly(t, seaTarget, friendlyWater, Matches.TerritoryIsWater, false);
                                    if(seaRoute == null)
                                        continue;
...
//					s_logger.fine("Attacking: "+largestGroupTerr.getName()+"; Ship Strength: "+shipStrength);
					...
					if (thisAttackStrength > maxStrength) //what happens if we knock out the biggest group?
					{
						...
						if (attackGroup)
						{
							s_logger.finer("SCM moving " + xMoves2 + " to attack threat on capital: " +xRoutes2);
