...
public class StrongAI extends AbstractAI implements IGamePlayer, ITripleaPlayer
{
	...
	private Route getAmphibRoute(final PlayerID player, final boolean nonCombat)
	{
		...
		Route route = SUtils.findNearest(ourCapitol, endMatch, Matches.TerritoryIsWater, data); // If we couldn't find an enemy free ncm sea route, just check for any water route
	...
	private boolean isAmphibAttack(PlayerID player, boolean requireWaterFactory)
	{
		...
		{
			List<Territory> factories = SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory);
		...
		if (amphibPlayer)
		{
			List<Territory> allFactories = SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory);
	...
	private void checkUnmovedTransports(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		List<Territory> ourFactories = SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory);
		...
		CompositeMatch<Territory> enemyLand = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotNeutral, Matches.territoryHasEnemyUnits(player, data),
					Matches.TerritoryHasProductionValueAtLeast(1));
	...
	private void populateTransportLoad(boolean nonCombat, GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, List<Collection<Unit>> transportsToLoad, PlayerID player)
	{
		...
		if (transTerr.isEmpty())
			return;
		List<Territory> myTerritories = new ArrayList<Territory>();
		...
		*/
		for (Territory checkThis : myTerritories)
		{
			...
			List<Unit> unitsTmp = checkThis.getUnits().getMatches(new CompositeMatchAnd<Unit>(landUnit, Matches.unitHasNotMoved));
			List<Unit> unitsToLoad = SUtils.sortTransportUnits(unitsTmp);
			...
			if (isLand)
			{
				CompositeMatch<Territory> noWaterEnemy = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotImpassableToLandUnits(player),
							Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data));
	...
	private void specialTransportMove(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		CompositeMatch<Territory> landPassable = new CompositeMatchAnd<Territory>(Matches.TerritoryIsLand, Matches.TerritoryIsPassableAndNotRestricted(player));
		CompositeMatch<Territory> enemyEmpty = new CompositeMatchAnd<Territory>(Matches.isTerritoryEnemy(player, data), Matches.territoryHasEnemyLandUnits(player, data).invert(), landPassable,
	...
	private void defendTransportingLocations(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		CompositeMatch<Territory> endCondition2 = new CompositeMatchAnd<Territory>(Matches.territoryHasLandRouteToEnemyCapital(data, player),
		...
		CompositeMatch<Unit> enemyAttackStuff = new CompositeMatchAnd<Unit>(Matches.unitIsEnemyOf(data, player), Matches.unitCanAttack(player));
		Territory capitol = TerritoryAttachment.getCapital(player, data);
		...
		if (Matches.TerritoryIsWater.match(startTerr) && Matches.territoryHasNoEnemyUnits(player, data).match(startTerr) && startNeededStrength > 0)
		{
			...
			CompositeMatch<Unit> ourWarshipsZ = new CompositeMatchAnd<Unit>(ourWarships, Matches.unitIsInTerritory(startTerr).invert(), Matches.unitIsInTerritory(firstEndTerr).invert(), Matches
			...
			{
				CompositeMatch<Unit> ourWarshipsX = new CompositeMatchAnd<Unit>(ourWarships);
		...
		{
			// check to see if there is no way we can win if we move there
			CompositeMatch<Unit> ourWarshipsZ = new CompositeMatchAnd<Unit>(ourWarships, Matches.unitIsInTerritory(firstEndTerr).invert(), Matches.unitIsInTerritory(endTerr).invert());
			...
			{
				List<Unit> myWarships = firstEndTerr.getUnits().getMatches(ourWarships);
				Collections.shuffle(myWarships); // we don't always want to have the weakest units stay first, in case we may need those destroyers for anti sub warfare
				// float myWarshipsStrength = SUtils.strength(myWarships, false, true, false);
				Iterator<Unit> shipIter = myWarships.iterator();
		...
		{
			// check to see if there is no way we can win if we move there
			CompositeMatch<Unit> ourWarshipsZ = new CompositeMatchAnd<Unit>(ourWarships, Matches.unitIsInTerritory(endTerr).invert());
	...
	private void populateTransportMove(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		*/
		...
		List<Territory> occTransTerr = SUtils.findTersWithUnitsMatching(data, player, transportingUnit);
		// not used
				...
				if (tTerrNeighbors.contains(targetTerr) && Matches.isTerritoryEnemy(player, data).match(targetTerr))
				{
					...
					{ // discourage invasions that don't have staying potential...add 1/5 of return potential attack
						...
						if (ourQuickStrength == 0.0F)
							continue;
						List<Unit> qAMoved = new ArrayList<Unit>(unitsAlreadyMoved);
						...
						if (ourQuickStrength < (eShortStrength + 0.25F * eShortPotential))
						{
							...
							qLandStrength = SUtils.inviteLandAttack(false, targetTerr, goStrength - ourQuickStrength, qAMoved, qunitMoved, qRoutes, data, player, false, Matches
										.territoryHasEnemyFactoryNeighbor(data, player).match(targetTerr),
										alreadyAttacked);
	...
	private void populateTransportUnload(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		Territory eTerr[] = new Territory[size]; // revised game has 79 territories and 64 sea zones
		...
		CompositeMatch<Unit> transporting = new CompositeMatchAnd<Unit>(Matches.UnitIsTransport, Matches.transportIsTransporting());
	...
	private void populateNonCombatSea(boolean nonCombat, final GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		Match<Unit> HasntMoved2 = new CompositeMatchAnd<Unit>(HasntMoved, notAlreadyMoved);
		Match<Unit> enemySeaUnit = new CompositeMatchAnd<Unit>(Matches.UnitIsSea, Matches.enemyUnit(player, data));
		...
		Match<Unit> carrierCanMove = new CompositeMatchAnd<Unit>(HasntMoved, ownedAC);
	...
	private void populateCombatMoveSea(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		CompositeMatch<Unit> alliedSeaAirUnitNotOwned = new CompositeMatchOr<Unit>(alliedSeaAirUnit, ownedUnit);
		...
		Territory myCapital = m_myCapital;
		...
		if (attackShipsPurchased)
		{
			...
			{ // if purchasing didn't specify the factory, figure it out
				for (Territory bestTarget : SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory))
				{
			...
			if (myCapital != null && seaPlaceFact != null)
			{
				...
				if (largestGroupTerr != null)
				{
					...
					if (thisAttackStrength > maxStrength) // what happens if we knock out the biggest group?
					{
						...
						else if (weWin)
						{
							...
							int remainingShips = totUnitCount - (maxUnitCount - enemyUnits.totalValues());
				...
				if (maxShipsTerr != null) // This whole code block was only supposed to do stuff if maxShipsTerr was not-null, so just skip it if the var is null(even though it doesn't cause errors)
				{
					...
					for (Territory xT : shipTerrAtCapitol)
					{
						...
						Route amphiRoute = getAmphibRoute(player, false);
	...
	{
		final BattleDelegate delegate = DelegateFinder.battleDelegate(data);
		Match<Territory> canLand = new CompositeMatchAnd<Territory>(Matches.isTerritoryAllied(player, data), new Match<Territory>()
		{
			@Override
			public boolean match(Territory o)
			{
		...
		Match<Territory> routeCondition = new CompositeMatchAnd<Territory>(Matches.territoryHasEnemyAA(player, data).invert(), Matches.TerritoryIsPassableAndNotRestricted(player));
	...
	private void populateNonCombat(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		CompositeMatchAnd<Territory> moveThrough = new CompositeMatchAnd<Territory>(Matches.TerritoryIsPassableAndNotRestricted(player), Matches.TerritoryIsNotNeutral, Matches.TerritoryIsLand);
		...
		for (Territory t : alliedTerr)
		{
			...
			CompositeMatch<Territory> enemyPassableNotWater = new CompositeMatchAnd<Territory>(Matches.isTerritoryEnemy(player, data), Matches.TerritoryIsPassableAndNotRestricted(player),
			...
			*/
			...
			List<Unit> moveableUnits = new ArrayList<Unit>(units);
			moveableUnits.removeAll(alreadyMoved);
	...
	private void movePlanesHomeNonCom(List<Collection<Unit>> moveUnits, List<Route> moveRoutes, final PlayerID player, GameData data)
	{
		...
		Matches.isTerritoryAllied(player, data), new Match<Territory>()
		{
			@Override
			public boolean match(Territory o)
			{
	...
	private void populateCombatMove(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		*/
		...
		CompositeMatch<Unit> attackable = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitIsLand, Matches.UnitIsNotAA, Matches.UnitIsNotStatic(player),
		...
		CompositeMatch<Unit> alliedLandUnit = new CompositeMatchAnd<Unit>(alliedNotOwned, Matches.UnitIsLand, Matches.UnitIsNotAA, Matches.UnitIsNotFactory);
		...
		CompositeMatch<Territory> enemyTerr = new CompositeMatchAnd<Territory>(Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data),
		...
		CompositeMatch<Unit> moveableFactory = new CompositeMatchAnd<Unit>(ourFactory, Matches.UnitIsNotStatic(player));
		...
		if (!moveableFactoryTerritories.isEmpty())
		{
			...
			CompositeMatch<Territory> goodFactTerr = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotImpassableToLandUnits(player), Matches.isTerritoryOwnedBy(player));
			for (Territory moveableFactoryTerr : moveableFactoryTerritories)
			{
				...
				{
					List<Territory> goodNeighbors = new ArrayList<Territory>(data.getMap().getNeighbors(moveableFactoryTerr, goodFactTerr));
	...
	private void populateFinalTransportUnload(GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, PlayerID player)
	{
		...
		Match<Unit> transports = new CompositeMatchAnd<Unit>(Matches.transportIsTransporting(), myTransport);
	...
	private void populateFinalMoveToCoast(final GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, final PlayerID player)
	{
		...
		for (Territory ter : data.getMap().getTerritoriesOwnedBy(player))
		{
			...
			List<Unit> unmovedUnits = ter.getUnits().getMatches(
			...
			List<Unit> landThreats = SUtils.findAttackers(ter, 25, new HashSet<Integer>(), player, data, new CompositeMatchAnd<Unit>(Matches.UnitIsLand, Matches.unitIsEnemyOf(data, player)),
	...
	private void populateMoveUnusedTransportsToFillLocation(final GameData data, List<Collection<Unit>> moveUnits, List<Route> moveRoutes, final PlayerID player)
	{
		...
		Match<Unit> unusedTransportMatch = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitIsTransport, Matches.transportIsNotTransporting(), Matches.unitHasMovementLeft,
	...
	protected void purchase(boolean purchaseForBid, int PUsToSpend, IPurchaseDelegate purchaseDelegate, GameData data, PlayerID player)
	{
		...
		CompositeMatch<Territory> enemyAndNoWater = new CompositeMatchAnd<Territory>(Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data),
					Matches.TerritoryIsNotImpassableToLandUnits(player));
		...
		for (Territory fT : factories)
		{
			...
			else
			{
				...
				Route minDistRoute = SUtils
							.findNearest(fT, Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data), Matches.TerritoryIsNotImpassableToLandUnits(player), data);
		...
		else if (player.getRepairFrontier() != null && games.strategy.triplea.Properties.getDamageFromBombingDoneToUnitsInsteadOfTerritories(data)) // figure out if anything needs to be repaired
		{
			...
			IntegerMap<RepairRule> repairMap = new IntegerMap<RepairRule>();
			HashMap<Unit, IntegerMap<RepairRule>> repair = new HashMap<Unit, IntegerMap<RepairRule>>();
			Collection<Unit> unitsThatCanProduceNeedingRepair = new ArrayList<Unit>();
			Collection<Unit> unitsThatAreDisabledNeedingRepair = new ArrayList<Unit>();
			CompositeMatchAnd<Unit> ourDisabled = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitIsDisabled());
			final int minimumUnitPrice = 3;
			int diff = 0;
			int totalDamage = 0;
			int capDamage = 0;
			...
			for (Territory fixTerr : rfactories)
			{
				...
				Unit possibleFactoryNeedingRepair = TripleAUnit.getBiggestProducer(Match.getMatches(fixTerr.getUnits().getUnits(), ourFactories), fixTerr, player, data, false);
			...
			if (player.getUnits().someMatch(Matches.UnitIsSea))
			{
				...
				if (seaRoute != null)
				{
					...
					if (checkSeaTerr != null)
					{
						...
						float bStrength = SUtils.strength(player.getUnits().getMatches(attackUnit), false, true, tFirst);
						float totStrength = aStrength + bStrength;
