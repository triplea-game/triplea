...
//				continue; //messing up here if we let it pick up at the Amphib Route...fix later
			...
			List<Territory> otherTerr = SUtils.getNeighboringLandTerritories(data, player, xT);
			List<Unit> transUnits = xT.getUnits().getMatches(transUnit);
			transUnits.removeAll(transportsFilled);
			if (transUnits.size() == 0)
				continue;
			if (otherTerr.size() == 1)
			{
				Territory thisTerr = otherTerr.get(0);
				List<Territory> badGuyCheck = SUtils.getNeighboringEnemyLandTerritories(data, player, thisTerr);
				if (badGuyCheck.size() > 0)
					continue;
				boolean isIsland = SUtils.doesLandExistAt(thisTerr, data);
				List<Unit> unitsToLoad = thisTerr.getUnits().getMatches(landUnit);
				if (unitsToLoad.size() == 0)
					continue;
				if (!isIsland)
				{ //load her up...if there are units here
					List<Unit> units = new ArrayList<Unit>();
					int transCount = transUnits.size();
	            	for (int j=transCount-1; j <= 0; j--)
					{
						transport = transUnits.get(j);
						int free = tracker.getAvailableCapacity(transport);
						if (free <=0)
						{
							transUnits.remove(j);
					...
					Route islandRoute = data.getMap().getRoute(thisTerr, xT);
					moveUnits.add(units);
					moveRoutes.add(islandRoute);
					transportsToLoad.add(transUnits);
				}
			}
			else
			{
				for (Territory xT2 : otherTerr)
				{
					Territory thisTerr = xT2;
					List<Territory> myEnemies = SUtils.getNeighboringEnemyLandTerritories(data, player, thisTerr);
					if (myEnemies.size() == 0)
					{
						List<Territory> tCheck = new ArrayList<Territory>();
						int howFar = SUtils.distanceToEnemy(xT2, tCheck, data, player);
						if (howFar == 0 || howFar > 4)
						{
							List<Unit> unitsToLoad = thisTerr.getUnits().getMatches(landUnit);
							if (unitsToLoad.size() == 0)
								continue;
...
//		transTerr2.removeAll(occTransTerr);
		...
		{
			Territory newTerr = SUtils.findASeaTerritoryToPlaceOn(fT, data, player);
			if (newTerr != null)
...
*/
		...
		for (Territory bGTerr : badGuyTerr)
		{
			if (bGTerr == null || bGTerr.isWater() || Matches.TerritoryIsNeutral.match(bGTerr))
				continue;
			float bGStrength = SUtils.strength(bGTerr.getUnits().getMatches(landOrAirEnemy), false, false);
...
//			maxPasses=1;
		...
		for (Territory eC : enemyCaps) //priority...send units into capitals & capneighbors when possible
		{
			...
			float capStrength = SUtils.strength(capUnits, false, false);
			for (Territory nT : neighborTerr)
			{
				if (nT.getUnits().someMatch(transportingUnit))
				{
					...
					float invadeStrength = SUtils.strength(specialLandUnits, true, false);
					float airStrength = 0.0F;
					for (Territory bTerr : bomberTerr)
					{
		...
		for (Territory t : transTerr) //complete check
		{
			...
			float ourStrength = SUtils.strength(units, true, false);
			if (units.size() == 0)
				continue;
			...
			{
				eTerr[i]=t2;
				eStrength[i]=SUtils.strength(t2.getUnits().getMatches(landOrAirEnemy), false, false);
				i++;
			}
	   ...
	   Match<Unit> notAlreadyMoved =new CompositeMatchAnd<Unit>(new Match<Unit>()
		...
		Match<Unit> HasntMoved2 = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), HasntMoved, notAlreadyMoved);
       	...
       	for (Territory AttackFrom : myAirTerr)
       	{
			...
			float myFighterStrength = SUtils.strength(myFighters, true, false);
			List<Unit> myBombers = AttackFrom.getUnits().getMatches(bomberUnit);
			float myBomberStrength = SUtils.strength(myBombers, true, false);
			...
			if (enemyNeighbors != null)
			{
				for (Territory badGuys : enemyNeighbors)
				{
					...
					if (badGuys.isWater())
						badGuyStrength = SUtils.strength(enemyUnits, false, true);
					else
						badGuyStrength = SUtils.strength(enemyUnits, false, false);
...
//				FtargetTerr = capRoute.getTerritories().get(2);
       ...
       {// are the remaining sea units massed into two or three groups? let's go get the largest one
		   ...
		   { //calculate using all allied units...the other guy can catch up later
			   ...
			   float enemySTstrength = SUtils.strength(enemySTunits, true, true); //obviously ignoring surrounding units
			   float ourSTstrength = SUtils.strength(ourSTunits, false, true);
			   ...
			   for (Territory closeTerr : closeTerrs)
			   {
				   ...
				   ourSTstrength += SUtils.strength(moreUnits, false, true);
			   }
			   ...
			   { //go ahead and move forward units that are on the border
				   ...
				   if (directRoute != null && directRoute.getLength() > 0)
				   {
				      ...
				      directRoute = data.getMap().getWaterRoute(goHere, newGoHere);
				      ...
				      for (Territory closeTerr2 : closeTerrs)
				      {
					     if (closeTerr2.isWater() && closeTerr2 != newGoHere)
					     {
					    	  ...
					    	  Route shortRoute = data.getMap().getWaterRoute(closeTerr2, newGoHere);
					    	  if (shortRoute != null)
					    	  {
		   ...
		   if (seaTarget != null && goHere != null) //more rigorous check and other units moved
		   {
		       ...
		       badGuyStrength = SUtils.strength(badGuys, true, true);
		       goUnits2 = goHere.getUnits().getMatches(seaAirAttackUnit);
		       getStrength = SUtils.strength(goUnits2, false, true);
		       goUnits4 = goHere.getUnits().getMatches(seaAirAttackUnitNotMoved);
		       SUtils.getStrengthAt(s1, s2, data, player, seaTarget, true, true);
			   ...
			   if (enemyPlayer != null) //otherwise...game is over
		       	  SUtils.getStrengthAt(e1, alliedStrength, data, enemyPlayer, seaTarget, true, true);
		       ...
		       if (pickedOne)
		       {
		          for (Territory ourTerr : seaAttackTerr)
		          {
			          ...
			          float xStrength = SUtils.strength(goUnits, false, true);
					  if (ourTerr != goHere)
					  {
       ...
       for(Territory t : data.getMap())
       {
		  ...
		  {
              routeToCapital = data.getMap().getWaterRoute(t, mySeaCapital);
          ...
          {
			   Route thisRoute = data.getMap().getWaterRoute(t, t3);
			   if (thisRoute != null)
			   {
          ...
          Route enemyRoute = data.getMap().getWaterRoute(t, myEnemy);
          ...
          if (distToEnemy == 2 && enemyRoute != null)
          {
          	  ...
          	  enemyRoute= data.getMap().getWaterRoute(t, stopTerr);
		  }
          if (distToEnemy == 1)
          {
		...
		for (Territory t : data.getMap())
		{
		   ...
		   if (tPlanes.size() > 0)
		   {
		   	   ...
		   	   for (Territory acT : acTerr)
		       {
				   ...
				   int availSpace = acUnits.size()*2 - airOnAC.size();
				   if (availSpace > 0)
				   {
...
//        TransportTracker tracker = DelegateFinder.moveDelegate(data).getTransportTracker();
		...
		for (Territory t: data.getMap())
		{
			...
			sortStrength[numTerr] = SUtils.strength(t.getUnits().getMatches(enemyAirSeaUnit), false, true);
			numTerr++;
		}
        ...
        for (int i=0; i< numTerr; i++)
        {
				...
				if (owned.isWater() & (rDist > -1) & (rDist <= 2) & (thisRoute != null)) //should clear out invalid water routes
				{
					if (MoveValidator.onlyAlliedUnitsOnPath(thisRoute, player, data))
					{
						...
						for (Unit u : allSeaUnits)
						{
							...
							{
								ourStrength += SUtils.uStrength(u, true, true);
								tmpUnits.add(u);
								shipsAttacked = true;
			...
			if (ourStrength < strengthNeeded && shipsAttacked) //discourage planes attacking when outnumbered
			{
				...
				for (Territory qAlliedCheck : alliedCheck)
				{
					...
					alliedStrength += SUtils.strength(qAlliedUnits, true, true);
				}
...
//        for (Territory fighterTerr : removeFromTerr) //we know that these must move

/*		List<Territory> acTerr = SUtils.ACTerritory(player, data); //put fighters on carriers
		for (Territory ac1 : acTerr)
		{
			List<Unit> acUnits = ac1.getUnits().getMatches(ACowned);
//			List<Unit> allFighters = ac1.getUnits().getMatches(Matches.UnitCanLandOnCarrier);
			List<Unit> myFighters = ac1.getUnits().getMatches(fighterUnit);
			int availSpace = acUnits.size()*2 - myFighters.size();
			if (availSpace == 0)
				continue;
			if (availSpace < 0) //move a fighter elsewhere
			{
				List<Unit> tmpUnits = new ArrayList<Unit>();
				Route goRoute = SUtils.findNearest(ac1, canLand, null, data);
				Territory xTerr = goRoute.getTerritories().get(goRoute.getLength());
				int goDist = data.getMap().getDistance(ac1, xTerr);
				boolean foundASpot = false;
				availSpace = 0 - availSpace;
				for (Unit xUnit : myFighters)
				{
					if (availSpace > 0 && MoveValidator.hasEnoughMovement(xUnit, goDist))
					{
						tmpUnits.add(xUnit);
						availSpace--;
					}
				}
				if (availSpace == 0 && goRoute != null)
				{
			...
			List<Territory> airTerr = SUtils.findOurPlanes(ac1, data, player, fighterUnit);
			for (Territory t3 : airTerr)
			{
				if (ac1 == t3)
					continue;
				int rDist = data.getMap().getDistance(t3, ac1);
				if (rDist > 4)
					continue;
				List<Unit> airUnits = t3.getUnits().getMatches(fighterUnit);
				boolean acUnitsHere = t3.getUnits().someMatch(Matches.UnitIsCarrier);
				if (acUnitsHere) //no reason to swap fighters with another AC
					continue;

				List<Unit> airUnitsAdd = new ArrayList<Unit>();
				for (Unit u : airUnits)
				{
					if (!unitsAlreadyMoved.contains(u) && MoveValidator.hasEnoughMovement(u, rDist))
						airUnitsAdd.add(u);
				}
//				airUnitsAdd.addAll(airUnits);
				if (airUnitsAdd.size() > 0)
				{
					Route airRoute = data.getMap().getRoute(t3, ac1);
					moveRoutes.add(airRoute);
					moveUnits.add(airUnitsAdd);
					unitsAlreadyMoved.addAll(airUnitsAdd);
//					airUnitsAdd.clear();
				}
			}
		}
...
//		List<Territory> anyNearby = SUtils.getNeighboringEnemyLandTerritories(data, player, myCapital);
		...
		float totalInvasion = SUtils.getStrengthOfPotentialAttackers(myCapital, data, player);
        ...
        for(Territory t : territories)
        {
            ...
            realStrength = SUtils.strength(unitsHere, false, false) - SUtils.allairstrength(unitsHere, false);

            ourStrength = SUtils.strength(unitsHere, false, false);
            attackerStrength = SUtils.getStrengthOfPotentialAttackers( t, data, player );
...
//				ourStrength += 15.0F; //assume new units on the way
 				if (totalInvasion > ourStrength) //borrow some units
				{
					...
					{ //get everything
						...
						for (Unit x2 : allUnits)
						{
							...
							{
								ourStrength += SUtils.uStrength(x2, false, false);
								sendIn2.add(x2);
							}
						}
            ...
            e1 = 0.0F;
            e2 = 0.0F;
            SUtils.getEnemyStrengthAt(e1, e2, data, player, myCapital, false, true);
            SUtils.getStrengthAt(s1, s2, data, player, myCapital, false, true); //doesn't include adjustments already done
					...
					if (retreatRoute != null)
					{
						...
						for (int i3=0; i3 < totUnits2; i3++)
						{
							if (s1 < e1*1.15F)
							{
								...
								s1 += SUtils.uStrength(moveThisUnit, true, false);
...
//		output.format("%s", "We Could Attack: "+enemyOwned+"/n");
		...
		for (Territory t: enemyOwned) //rank the problems
		{
            ...
            if(!t.getUnits().someMatch(Matches.enemyUnit(player, data) ) )
				...
				sortStrength[numTerr] = SUtils.strength(t.getUnits().getUnits(), false, false);
			numTerr++;
		}
		for (int i2=1; i2 <=(numTerr - 2); i2++)
		{
 		...
 		for (Territory tProb: bigProblem2) //rank the problems
		{
            ...
            if(!tProb.getUnits().someMatch(Matches.enemyUnit(player, data) ) )
				...
				sortStrengthProblems[numTerrProblem] = SUtils.strength(tProb.getUnits().getUnits(), false, false);
...
//		output.format("%s", "Big Problems: " + bigProblem2+"\n");
		...
		if (!ownMyCapital) //attack the capital with everything we have
		{
            ...
            float badCapStrength = SUtils.strength(myCapital.getUnits().getUnits(), false, false);
            float capStrength = 0.0F;
            for (Territory checkCap : attackFrom)
            {
                List<Unit> units = checkCap.getUnits().getMatches(attackable);
                capStrength += SUtils.strength(units, true, false);
			}
			...
			if (alreadyAttacked.contains(myCapital)) //only use planes if we have already sent in ground units
			{
				for (Territory everyWhere : planeTerr)
				{
					if (Matches.territoryHasEnemyUnits(player, data).match(everyWhere))
						continue;
					List<Unit> tmpUnits = new ArrayList<Unit>();
					Route thisRoute = data.getMap().getRoute(everyWhere, myCapital);
		...
		for (Territory badCapitol : enemyCaps)
		{
			...
			float badCapStrength = SUtils.strength(badCapUnits, false, false);
			...
			for (Territory aT : alliedCapTerr)
			{
				...
				alliedCapStrength += SUtils.strength(alliedCUnits, true, false);
				ourCapStrength += SUtils.strength(ourCUnits, true, false);
			}
        ...
        for(Territory enemy : enemyOwned)
        {
			if (alreadyAttacked.contains(enemy))
				continue;
			float eStrength = SUtils.strength(enemy.getUnits().getUnits(), true, false);
            if( eStrength <= 0.5F)
            {
		...
		for (Territory badTerr : bigProblem2)
		{
			...
			badStrength = SUtils.strength(enemyUnits, false, false);
            ...
            if(badStrength > 0.0F)
            {
                ...
                {
					float thisAttack = SUtils.strength(capSavers.getUnits().getMatches(attackable), true, false);
                ...
                if(ourStrength > badStrength*attackFactor2)
                {
                    ...
                    float maxAttackFactor = 2.12F;
                    if (bigProblem2.size() > 1)
                    	maxAttackFactor = 1.65F;//concerned about overextending if more than 1 territory
                    ...
                    for (Territory owned : capitalAttackTerr )
                    {
						...
						for (Unit capAttackUnit : units3)
						{
							if (remainingStrengthNeeded < 0.0F)
								continue;
							remainingStrengthNeeded -= SUtils.uStrength(capAttackUnit, true, false);
        ...
        //find the territories we can reasonably expect to take
 		...
 		float capStrength = SUtils.strength(myCapital.getUnits().getMatches(attackable), false, false); //defense strength
    	SUtils.getEnemyStrengthAt(e1, e2, data, player, myCapital, false, true);
    	SUtils.getStrengthAt(s1, s2, data, player, myCapital, false, true);
        ...
        for(Territory enemy : enemyOwned)
        {
			if (alreadyAttacked.contains(enemy))
				continue;
            float enemyStrength = SUtils.strength(enemy.getUnits().getUnits(), false, false);
            if(enemyStrength > 0.0F)
            {
                ...
                {
                	ourStrength += SUtils.strength(checkTerr2.getUnits().getMatches(attackable), true, false);
                	alliedStrength += SUtils.strength(checkTerr2.getUnits().getMatches(alliedAirLandUnit), true, false);
				}
                if (ourStrength <= enemyStrength*attackFactor)
                	continue;

                remainingStrengthNeeded = (2.15F * enemyStrength) + 5.0F; //limit the attackers
				if (attackFrom.size() == 1) //if we have 1 big attacker
				{
					...
					if (enemyLandTerr.size() == 1) //the only enemy territory is the one we are attacking
					    remainingStrengthNeeded = (2.65F * enemyStrength) + 5.0F; //blow it away
                ...
                for(Territory owned : attackFrom )
                {
					...
					for (Unit AttackUnit : ourUnits)
					{
						if (remainingStrengthNeeded < 0.0F)
							continue;
						remainingStrengthNeeded -= SUtils.uStrength(AttackUnit, true, false);
...
//        pause();
        ...
        for (Territory eT : enemyAttackShipTerr)
        	enemySeaStrength += SUtils.strength(eT.getUnits().getMatches(enemyAttackShip), true, true);

		for (Territory oT : ourAttackShipTerr)
			ourSeaStrength += SUtils.strength(oT.getUnits().getMatches(ourAttackShip), true, true);
        ...
        SUtils.getStrengthAt(ourSea1, ourSea2, data, player, ourCapital, true, true);
        SUtils.getStrengthAt(ourLand1, ourLand2, data, player, ourCapital, false, true);
        SUtils.getEnemyStrengthAt(enemySea1, enemySea2, data, player, ourCapital, true, true);
        SUtils.getEnemyStrengthAt(enemyLand1, enemyLand2, data, player, ourCapital, false, true);
		...
		for (int i=0; i<=3; i++)
		{
            ...
            for(ProductionRule rule : rules)
            {
                ...
                if (Matches.UnitTypeIsAir.match(results))
                {
					...
					if (buyOnePlane && unitCount < totIPC)
					{
						purchase.add(rule, 1);
						buyOnePlane= false;
						leftToSpend -= cost;
...
//					continue;
		...
		for (int j=0; j<=3; j++) //make 3 passes on sea purchases
		{
            ...
            for(ProductionRule rule : rules)
            {
                ...
                if ((leftToSpend < cost || ipcSea < cost || unitCount >= totIPC))
                	continue;
				...
				if (doBuyAttackShips && unitCount < totIPC && ipcSea >= cost && transportCapacity > 0)
				{
				...
				if (ipcSea >= cost && cost < 13 && unitCount < totIPC && Math.random() < 0.28 && (doBuyAttackShips==(transportCapacity < 0)))
				{
					ipcSea-=cost;
					leftToSpend-=cost;
					purchase.add(rule, 1);
				...
				if (ipcSea >= cost && (cost >= 13 && cost < 20) && unitCount < totIPC && Math.random() < 0.18)
				{
					ipcSea-=cost;
					leftToSpend-=cost;
					purchase.add(rule, 1);
    ...
    protected void place(boolean bid, IAbstractPlaceDelegate placeDelegate, GameData data, PlayerID player)
    {
        ...
        */
		...
		Territory seaPlaceAtTrans = SUtils.findASeaTerritoryToPlaceOn(factUnitTerr, data, player);
		minDist = 100;
		...
		Territory seaPlaceAtAttack = SUtils.findASeaTerritoryToPlaceOn(thisFactTerr, data, player);
        ...
        {
			Territory seaPlaceAt = SUtils.findASeaTerritoryToPlaceOn(t, data, player);
			if (seaPlaceAt == null)
    ...
    public CasualtyDetails selectCasualties(Collection<Unit> selectFrom, Map<Unit, Collection<Unit>> dependents, int count, String message, DiceRoll dice, PlayerID hit, List<Unit> defaultCasualties, GUID battleID)
    {
		...
		if (xCount > 0)
		{
			...
			{
				if (Matches.UnitIsTransport.match(unitx) && !tracker.isTransporting(unitx) )
					workUnits.add(unitx);
			}
			for (Unit unitx : selectFrom)
			{
