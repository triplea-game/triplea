...
public class StrongAI extends AbstractAI implements IGamePlayer, ITripleaPlayer
{
	...
	private boolean isAmphibAttack(final PlayerID player, final boolean requireWaterFactory)
	{
		...
		{
			final List<Territory> factories = SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory);
		...
		if (amphibPlayer)
		{
			final List<Territory> allFactories = SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory);
	...
	private void planesToCarriers(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		final CompositeMatch<Territory> noNeutralOrAA = new CompositeMatchAnd<Territory>(SUtils.TerritoryIsNotImpassableToAirUnits(data), Matches.territoryHasEnemyAA(player, data).invert());
	...
	private void checkUnmovedTransports(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		final CompositeMatch<Unit> ourLandUnit = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitIsLand, Matches.UnitIsNotFactory);
				...
				final List<Collection<Unit>> transportsToLoad, final PlayerID player)
	{
		...
		final CompositeMatch<Unit> landUnit = new CompositeMatchAnd<Unit>(owned, Matches.UnitCanBeTransported, Matches.UnitIsNotAA, Matches.UnitIsNotFactory);
		...
		*/
		for (final Territory checkThis : myTerritories)
		{
			...
			if (isLand)
			{
				final CompositeMatch<Territory> noWaterEnemy = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotImpassableToLandUnits(player),
							Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data));
				final CompositeMatch<Territory> noWaterAllied = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotImpassableToLandUnits(player), Matches.isTerritoryAllied(player, data));
				final Route badGuyDR = SUtils.findNearest(checkThis, noWaterEnemy, noWaterAllied, data);
				final Route badGuyFactory = SUtils.findNearest(checkThis, Matches.territoryHasEnemyFactory(data, player), Matches.TerritoryIsNotImpassableToLandUnits(player), data);
	...
	private void quickTransportUnload(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		final CompositeMatch<Territory> landPassable = new CompositeMatchAnd<Territory>(Matches.TerritoryIsLand, Matches.TerritoryIsPassableAndNotRestricted(player));
		...
		for (final Territory enemyTerr : emptyEnemyTerrs)
		{
			...
			while (lNIter.hasNext())
			{
				...
				if (eAttackStrength > (defenseStrength * 1.10F - 2.0F) || (Matches.territoryHasAlliedFactory(data, player).match(thisTerr) && eAttackStrength > defenseStrength))
				{
					lNIter.remove();
					continue;
				}
				...
				{
					final Set<Territory> myFactNeighbors = data.getMap().getNeighbors(thisTerr, Matches.territoryHasAlliedFactory(data, player));
	...
	private void firstTransportMove(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		{ // don't move transports which are next to a good spot
			...
			if (!neighborList.isEmpty())
				continue;
			final List<Territory> neighborList2 = SUtils.getExactNeighbors(myTerr, 2, player, true);
		...
		*/
		for (final Territory transTerr : occTransTerr)
		{
			...
			while (tTIter.hasNext())
			{
				...
				if (Matches.TerritoryIsWater.match(tmpTerr) || Matches.TerritoryIsPassableAndNotRestricted(player).invert().match(tmpTerr)
				...
				if (tTerrNeighbors.contains(targetTerr) && Matches.isTerritoryEnemy(player, data).match(targetTerr))
				{
					...
					{ // discourage invasions that don't have staying potential...add 1/5 of return potential attack
						...
						if (ourQuickStrength < (eShortStrength + 0.25F * eShortPotential))
						{
							...
							qLandStrength = SUtils.inviteLandAttack(false, targetTerr, goStrength - ourQuickStrength, qAMoved, qunitMoved, qRoutes, data, player, false, Matches
										.territoryHasEnemyFactoryNeighbor(data, player).match(targetTerr), alreadyAttacked);
	...
	private void populateTransportUnload(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		final CompositeMatch<Unit> enemyfactories = new CompositeMatchAnd<Unit>(Matches.UnitIsFactory, enemyUnit);
		...
		for (final Territory t : transTerr) // complete check
		{
			...
			for (int j2 = 0; j2 < i - 1; j2++) // sort the territories by strength
			{
				...
				final Set<Territory> badFactTerr = data.getMap().getNeighbors(tmpTerr, Matches.territoryHasEnemyFactory(data, player));
	...
	*/
	...
	private void specialPlaneAttack(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		final CompositeMatch<Territory> noEnemyAA = new CompositeMatchAnd<Territory>(Matches.territoryHasEnemyAA(player, data).invert(), Matches.TerritoryIsNotImpassable);
	...
	{
		final CompositeMatch<Unit> landUnit = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitIsLand, Matches.UnitIsNotAA, Matches.UnitIsNotFactory);
			...
			for (final Territory testCap : threats)
			{
				...
				for (final Territory moveFrom : copyOne)
				{
					if (!moveFrom.isWater() && moveFrom.getUnits().someMatch(landUnit))
					{
						...
						final Route aRoute = data.getMap().getRoute(moveFrom, testCap, Matches.territoryHasEnemyAA(player, data).invert());
						if (aRoute != null)
						{
	...
	private void populateNonCombatSea(final boolean nonCombat, final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		final Match<Unit> alliedAirAttackUnit = new CompositeMatchAnd<Unit>(Matches.alliedUnit(player, data), Matches.UnitIsAir);
		final Match<Unit> alliedSeaAirAttackUnit = new CompositeMatchOr<Unit>(alliedSeaAttackUnit, alliedAirAttackUnit);
		final Match<Territory> noNeutralOrAA = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotImpassable, Matches.territoryHasEnemyAA(player, data).invert());
		...
		for (final Territory enemy : enemyTerr)
		{
			...
			for (final Territory shipTerr : ourShipTerrs)
			{
				...
				final List<Territory> Neighbors2 = SUtils.getExactNeighbors(shipTerr, 2, player, false);
	...
	private void nonCombatPlanes(final GameData data, final PlayerID player, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes)
	{
		...
		final List<Unit> unitsAlreadyMoved = new ArrayList<Unit>();
		final CompositeMatch<Territory> notNeutralOrAA = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotImpassable, Matches.territoryHasEnemyAA(player, data).invert());
		// Territory myCapital = TerritoryAttachment.getCapital(player, data);
		// boolean capDanger = markFactoryUnits(data, player, unitsAlreadyMoved);
		final boolean tFirst = transportsMayDieFirst();
	...
	private void populateCombatMoveSea(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		if (attackShipsPurchased)
		{
			...
			{ // if purchasing didn't specify the factory, figure it out
				for (final Territory bestTarget : SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory))
				{
	...
	*/
	...
	// we've already check Carriers in moveNonComPlanes
	{
		...
		final Match<Territory> routeCondition = new CompositeMatchAnd<Territory>(Matches.territoryHasEnemyAA(player, data).invert(), Matches.TerritoryIsPassableAndNotRestricted(player));
	...
	private void stopBlitzAttack(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		final CompositeMatch<Unit> blitzBlocker = new CompositeMatchAnd<Unit>(Matches.alliedUnit(player, data), Matches.UnitIsNotFactory, Matches.UnitIsNotAA);
		final CompositeMatch<Unit> anyUnit = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitIsNotFactory, Matches.UnitIsNotAA);
		...
		if (TerrToBlock.size() > 0) // still more
		{
			for (final Territory xTerr : goBlockTerr)
			{
				...
				final List<Unit> alliedUnits = xTerr.getUnits().getMatches(alliedUnit);
	...
	private void populateNonCombat(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		{
			final List<Territory> outerTerrs = SUtils.getExactNeighbors(myCapital, 3, player, false);
		...
		for (final Territory t : alliedTerr)
		{
			...
			final CompositeMatch<Territory> enemyPassableNotWater = new CompositeMatchAnd<Territory>(Matches.isTerritoryEnemy(player, data), Matches.TerritoryIsPassableAndNotRestricted(player),
						Matches.TerritoryIsLand);
			final CompositeMatch<Territory> enemyPassableNotWaterNotNeutral = new CompositeMatchAnd<Territory>(enemyPassableNotWater, Matches.TerritoryIsNotNeutral);
			final CompositeMatch<Territory> routeCondition = new CompositeMatchAnd<Territory>(Matches.TerritoryIsPassableAndNotRestricted(player), Matches.isTerritoryAllied(player, data));
	...
	private void secondNonCombat(final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player, final GameData data)
	{
		...
		final List<Territory> myFactories = Match.getMatches(SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory), Matches.isTerritoryAllied(player, data));
		...
		final CompositeMatch<Territory> endCondition = new CompositeMatchAnd<Territory>(Matches.territoryHasEnemyUnits(player, data), Matches.TerritoryIsNotNeutral, Matches.TerritoryIsLand);
		...
		{// TODO: find another territory to join if possible
			...
			{
				final Route closestERoute = SUtils.findNearest(ownedTerr, endCondition, Matches.TerritoryIsNotImpassableToLandUnits(player), data);
	...
	private void movePlanesHomeNonCom(final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player, final GameData data)
	{
		...
		final CompositeMatch<Unit> alliedFactory = new CompositeMatchAnd<Unit>(Matches.alliedUnit(player, data), Matches.UnitIsFactory);
		final CompositeMatch<Unit> fighterUnit = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitCanLandOnCarrier);
		...
		final CompositeMatch<Territory> noNeutralOrAA = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotNeutral, Matches.territoryHasEnemyAA(player, data).invert(),
					Matches.TerritoryIsNotImpassable);
		final List<Unit> alreadyMoved = new ArrayList<Unit>();
		...
		for (final Territory tFight : badPlaneTerrs)
		{
			...
			{// each group will have an identical movement
				...
				while (needToLand > 0 && lzIter.hasNext() && !fighterGroup.isEmpty())
				{
					...
					if (Matches.TerritoryIsPassableAndNotRestricted(player).match(landingZone) && MoveValidator.canLand(fighterGroup, landingZone, player, data))
					{
	...
	private void populateCombatMove(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		*/
		...
		final CompositeMatch<Unit> attackable = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitIsLand, Matches.UnitIsNotAA, Matches.UnitIsNotStatic(player),
					Matches.UnitIsNotFactory, new Match<Unit>()
					{
						@Override
						public boolean match(final Unit o)
						{
		...
		final CompositeMatch<Unit> alliedLandUnit = new CompositeMatchAnd<Unit>(alliedNotOwned, Matches.UnitIsLand, Matches.UnitIsNotAA, Matches.UnitIsNotFactory);
		final CompositeMatch<Unit> alliedAirLandUnitNotOwned = new CompositeMatchOr<Unit>(alliedAirUnit, alliedLandUnit);
		final CompositeMatch<Unit> blitzUnit = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitCanBlitz);
		final CompositeMatch<Unit> enemyLandAirUnit = new CompositeMatchAnd<Unit>(Matches.UnitIsNotSea, Matches.UnitIsNotAA, Matches.UnitIsNotFactory);
		...
		final CompositeMatch<Unit> ourFactory = new CompositeMatchAnd<Unit>(ownedUnit, Matches.UnitIsFactory);
		...
		if (!moveableFactoryTerritories.isEmpty())
		{
			...
			final List<Territory> existingFactories = SUtils.findTersWithUnitsMatching(data, player, Matches.UnitIsFactory);
			...
			final CompositeMatch<Territory> goodFactTerr = new CompositeMatchAnd<Territory>(Matches.TerritoryIsNotImpassableToLandUnits(player), Matches.isTerritoryOwnedBy(player));
				...
				if (moveableFactories.size() > 0 && moveableFactoryTerr.getUnits().getMatches(ourFactory).size() > 1)
				{
					...
					for (final Territory moveFactToTerr : goodNeighbors)
					{
						...
						if (SUtils.hasLandRouteToEnemyOwnedCapitol(moveFactToTerr, player, data))
							territoryValue += 3;
						if (SUtils.findNearest(moveFactToTerr, Matches.territoryHasEnemyFactory(data, player), Matches.TerritoryIsNotImpassableToLandUnits(player), data) != null)
							territoryValue += 1;
		...
		final Route enemyRoute = SUtils.findNearest(maxAttackTerr, Matches.territoryHasAlliedFactory(data, player), Matches.TerritoryIsNotImpassableToLandUnits(player), data);
		if (enemyRoute != null)
		{
	...
	private void populateFinalMoveToCoast(final GameData data, final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final PlayerID player)
	{
		...
		for (final Territory ter : data.getMap().getTerritoriesOwnedBy(player))
		{
			...
			if (SUtils.findNearest(ter, new CompositeMatchAnd<Territory>(Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data), Matches.TerritoryIsLand),
						Matches.TerritoryIsNotImpassableToLandUnits(player), data) != null)
				continue;
	...
	protected void purchase(final boolean purchaseForBid, int PUsToSpend, final IPurchaseDelegate purchaseDelegate, final GameData data, final PlayerID player)
	{
		...
		final CompositeMatch<Unit> transportableUnit = new CompositeMatchAnd<Unit>(Matches.unitIsOwnedBy(player), Matches.UnitCanBeTransported, Matches.UnitIsNotAA, Matches.UnitIsNotFactory);
		...
		final CompositeMatch<Territory> enemyAndNoWater = new CompositeMatchAnd<Territory>(Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data),
					Matches.TerritoryIsNotImpassableToLandUnits(player));
		final CompositeMatch<Territory> noEnemyOrWater = new CompositeMatchAnd<Territory>(Matches.isTerritoryAllied(player, data), Matches.TerritoryIsNotImpassableToLandUnits(player));
		...
		if (ACCount > fighterCount)
			buyOnePlane = true;
		final Territory capitol = TerritoryAttachment.getCapital(player, data);
		final List<Territory> rfactories = SUtils.findUnitTerr(data, player, ourFactories);
		...
		*/
		...
		if ((unitCount < totProd) && leftToSpend > 0)
		{
			for (final ProductionRule quickProd : rules)
			{
				...
				if (Matches.UnitTypeIsSeaOrAir.match(intResults) || Matches.UnitTypeIsAAOrIsFactoryOrIsInfrastructure.match(intResults))
					continue;
	...
	protected void place(final boolean bid, final IAbstractPlaceDelegate placeDelegate, final GameData data, final PlayerID player)
	{
		...
		final CompositeMatch<Unit> ourFactory = new CompositeMatchAnd<Unit>(ownedUnit, Matches.UnitIsFactory);
		final CompositeMatch<Unit> landUnit = new CompositeMatchAnd<Unit>(ownedUnit, Matches.UnitIsLand, Matches.UnitIsNotFactory);
		...
		final Territory capitol = TerritoryAttachment.getCapital(player, data);
		final List<Territory> factoryTerritories = SUtils.findUnitTerr(data, player, ourFactory);
		...
		if (bid)
		{
			...
			if (player.getUnits().someMatch(Matches.UnitIsSea))
			{
				...
				final CompositeMatch<Territory> waterFactoryWaterTerr = new CompositeMatchAnd<Territory>(Matches.TerritoryIsWater, Matches.territoryHasOwnedFactoryNeighbor(data, player));
			...
			for (int i = 4; i >= 0; i--)
			{
				for (final Territory whichOne : landFactories)
				{
					if (landUnitFactories.getInt(whichOne) == i)
					{
						...
						{
							final Route whichRoute = SUtils.findNearest(whichOne, Matches.territoryHasEnemyLandNeighbor(data, player), Matches.TerritoryIsNotImpassableToLandUnits(player), data);
	...
	private void placeAllWeCanOn(final boolean bid, final GameData data, final Territory factoryPlace, final Territory placeAt, final IAbstractPlaceDelegate placeDelegate, final PlayerID player)
	{
		...
		{
			final Collection<Unit> toPlace = new ArrayList<Unit>(player.getUnits().getMatches(Matches.UnitIsFactory));
	...
	public Territory retreatQuery(final GUID battleID, final boolean submerge, final Collection<Territory> possibleTerritories, final String message)
	{
		...
		final Territory battleTerr = getBattleTerritory();
		final boolean attacking = false; // determine whether player is offense or defense
		...
		else
		{
			...
			final List<Unit> myShips = battleTerr.getUnits().getMatches(myShip);
			final List<Unit> myPlanes = battleTerr.getUnits().getMatches(myPlane);
			...
			final List<Unit> enemyAirUnits = battleTerr.getUnits().getMatches(enemyAirUnit);
			final List<Unit> enemySeaUnits = battleTerr.getUnits().getMatches(enemySeaUnit);
			...
			final IntegerMap<UnitType> myUnitList = SUtils.convertListToMap(myShips);
