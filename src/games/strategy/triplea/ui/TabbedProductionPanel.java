...
public class TabbedProductionPanel extends ProductionPanel {
  ...
  protected void initLayout(final PlayerID id) {
    ...
    if (dtab != null && dthis != null) {
      tabs.setPreferredSize(new Dimension(dtab.width + 4, dtab.height + 4)); // for whatever dumb reason, the tabs need
                                                                             // a couple extra
                                                                             // height and width or else scroll bars
                                                                             // will appear
      this.setPreferredSize(new Dimension(dthis.width + 8, dthis.height + 24)); // for whatever dumb reason, the window
                                                                                // needs to be at least
                                                                                // 16 pixels greater in height than
                                                                                // normal, to accommodate
                                                                                // the tabs
    }
    tabs.validate();
    this.validate();
  ...
  private void calculateXY(final ProductionTabsProperties properties, final int largestList) {
    if (properties == null || properties.getRows() == 0 || properties.getColumns() == 0
      ...
      m_columns = Math.max(3, properties.getColumns()); // There are small display problems if the size is less than 2x3
                                                        // cells.
    }
  }
  ...
  private List<Tuple<String, List<Rule>>> getDefaultRuleLists() {
    ...
    for (final Rule rule : m_rules) {
      ...
      if (resourceOrUnit instanceof UnitType) {
        ...
        if (attach.getIsConstruction()) { // canproduceUnits isn't checked on purpose, since this category is for units
                                          // that can be placed
