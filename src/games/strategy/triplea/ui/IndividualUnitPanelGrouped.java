...
public class IndividualUnitPanelGrouped extends JPanel {
   ...
   * @param showSelectAll
   */
  public IndividualUnitPanelGrouped(final Map<String, Collection<Unit>> unitsToChooseFrom, final GameData data, final IUIContext uiContext,
      final String title,
  ...
  private void layoutEntries() {
    ...
    for (final Entry<String, Collection<Unit>> entry : m_unitsToChooseFrom.entrySet()) {
      ...
      final IndividualUnitPanel chooser = new IndividualUnitPanel(possibleTargets, miniTitle, m_data, m_uiContext, m_max, m_showMinAndMax,
          m_showSelectAll, m_textFieldListener);
      ...
      chooserScrollPane.setPreferredSize(new Dimension(
          (chooserScrollPane.getPreferredSize().width > 220 ? 220
              : (chooserScrollPane.getPreferredSize().height > 520 ? chooserScrollPane.getPreferredSize().width + 20
                  : chooserScrollPane.getPreferredSize().width)),
          (chooserScrollPane.getPreferredSize().height > 520 ? 520
              : (chooserScrollPane.getPreferredSize().width > 220 ? chooserScrollPane.getPreferredSize().height + 20
    ...
    if (m_showSelectAll) {
      add(m_autoSelectButton,
          new GridBagConstraints(0, 2, 7, 1, 0, 0.5, GridBagConstraints.CENTER, GridBagConstraints.NONE, nullInsets, 0, 0));
      yIndex++;
    }
    add(m_leftToSelect,
        new GridBagConstraints(0, 3, 5, 2, 0, 0.5, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, nullInsets, 0, 0));
  }
