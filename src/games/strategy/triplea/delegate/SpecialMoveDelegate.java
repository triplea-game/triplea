...
public class SpecialMoveDelegate extends AbstractMoveDelegate implements IMoveDelegate
{
	...
	public String move(final Collection<Unit> units, final Route route, final Collection<Unit> transportsThatCanBeLoaded, final Map<Unit, Collection<Unit>> newDependents)
	{
		...
		final MoveValidationResult result = SpecialMoveDelegate.validateMove(units, route, player, transportsThatCanBeLoaded, newDependents, GameStepPropertiesHelper.isNonCombatMove(data, false),
					m_movesToUndo, data);
	...
	public static boolean allowAirborne(final PlayerID player, final GameData data)
	{
		...
		final Collection<PlayerID> alliesForBases = data.getRelationshipTracker().getAllies(player, true);
		final Collection<Territory> territoriesWeCanLaunchFrom = Match.getMatches(map.getTerritories(), Matches.territoryHasUnitsThatMatch(getAirborneMatch(player, airborneBases, alliesForBases)));
