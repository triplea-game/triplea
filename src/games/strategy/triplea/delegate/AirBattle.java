...
public class AirBattle extends AbstractBattle
{
	...
	class AttackersFire implements IExecutable
	{
		...
		public void execute(final ExecutionStack stack, final IDelegateBridge bridge)
		{
			...
			final IExecutable calculateCasualties = new IExecutable()
			{
				...
				public void execute(final ExecutionStack stack, final IDelegateBridge bridge)
				{
					m_details = BattleCalculator.selectCasualties(ATTACKERS_FIRE, m_defender, m_defendingUnits, m_battleSite, null, bridge, ATTACKERS_FIRE, m_dice, true, m_battleID, false,
								m_dice.getHits(), true);
	...
	class DefendersFire implements IExecutable
	{
		...
		public void execute(final ExecutionStack stack, final IDelegateBridge bridge)
		{
			...
			final IExecutable calculateCasualties = new IExecutable()
			{
				...
				public void execute(final ExecutionStack stack, final IDelegateBridge bridge)
				{
					m_details = BattleCalculator.selectCasualties(DEFENDERS_FIRE, m_attacker, m_attackingUnits, m_battleSite, null, bridge, DEFENDERS_FIRE, m_dice, false, m_battleID, false,
								m_dice.getHits(), true);
	...
	public static Match<Unit> defendingGroundSeaBattleInterceptors(final PlayerID attacker, final GameData data)
	{
		...
		return new Match<Unit>()
		{
			...
			public boolean match(final Unit u)
			{
				final CompositeMatch<Unit> canIntercept = new CompositeMatchAnd<Unit>(Matches.unitCanAirBattle, Matches.unitIsEnemyOf(data, attacker), Matches.UnitWasInAirBattle.invert());
	...
	public static Match<Unit> defendingBombingRaidInterceptors(final PlayerID attacker, final GameData data)
	{
		...
		return new Match<Unit>()
		{
			...
			public boolean match(final Unit u)
			{
				final CompositeMatch<Unit> canIntercept = new CompositeMatchAnd<Unit>(Matches.unitCanIntercept, Matches.unitIsEnemyOf(data, attacker), Matches.UnitWasInAirBattle.invert());
				if (!canScrambleIntoAirBattles)
					canIntercept.add(Matches.UnitWasScrambled.invert());
				return canIntercept.match(u);
			}
