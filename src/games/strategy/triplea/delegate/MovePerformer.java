...
public class MovePerformer implements Serializable {
    ...
    final IExecutable postAAFire = new IExecutable() {
      ...
      public void execute(final ExecutionStack stack, final IDelegateBridge bridge) {
        ...
        if (games.strategy.triplea.Properties.getUseFuelCost(data)) {
          change.add(markFuelCostResourceChange(units, route, id, data /* , createdBattle */)); // markFuelCostResourceChange
                                                                                                // must be done
                                                                                                // before we load/unload
                                                                                                // units
        }
  ...
  private Change markMovementChange(final Collection<Unit> units, final Route route, final PlayerID id) {
    ...
    final Iterator<Unit> iter = Match.getMatches(units, Matches.unitIsOwnedBy(id)).iterator(); // only units owned by us
                                                                                               // need to be marked
