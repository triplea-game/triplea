...
public class NCM_AirLandingCalculator
{
	public static Territory CalculateLandingLocationForAirUnits(GameData data, PlayerID player, Territory territory, List<Unit> airUnits, List<NCM_Task> tasks)
	{
		...
		for (Territory ter : data.getMap().getTerritories())
		{
			...
			float oldSurvivalChance = DUtils.GetSurvivalChanceOfArmy(data, player, ter, DUtils.GetTerUnitsAtEndOfTurn(data, player, ter),
			...
			Territory closestTerWithOurUnits = DUtils.GetClosestTerMatchingX(data, territory,
			...
			if (arePlanesFromCapsOrNeighbors)
			{
				...
				List<Float> capTakeoverChances = DUtils.GetTerTakeoverChanceBeforeAndAfterMove(data, player, ourClosestCap, ter, airUnits,
							DSettings.LoadSettings().CA_CMNCM_determinesIfTaskEndangersCap);
