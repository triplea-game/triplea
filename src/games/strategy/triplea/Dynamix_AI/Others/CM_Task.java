...
public class CM_Task
{
	...
	private void recruitEnoughUnitsForTradeTask(final float certaintyOfReachingLeftoverLUnitsGoalRequired, final boolean recruitTillTUVStartsDropping)
	{
		...
		for (final UnitGroup ug : sortedPossibles)
		{
			...
			final int tradeScore = DUtils.GetTaskTradeScore(m_data, m_target, attackers, defenders, simulatedAttack, responseAttackers, responseDefenders, simulatedResponse);
	...
	public boolean IsPlannedAttackWorthwhile(final List<CM_Task> allTasks)
	{
		...
		if (m_taskType == CM_TaskType.Land_Attack_Offensive || m_taskType == CM_TaskType.Land_Attack_Trade)
		{
			...
			final Territory closestToTargetEnemyCap = DUtils.GetClosestTerInList(data, DUtils.GetAllEnemyCaps_ThatAreOwnedByOriginalOwner(data, player), m_target);
			if (closestToTargetEnemyCap != null)
			{
				...
				final StrategyType strategyType = StrategyCenter.get(m_data, player).GetCalculatedStrategyAssignments().get(closestToTargetEnemyCap.getOwner());
		...
		final List<Unit> defenders = DUtils.ToList(m_target.getUnits().getMatches(Matches.unitIsEnemyOf(m_data, GlobalCenter.CurrentPlayer)));
		final AggregateResults simulatedAttack = DUtils.GetBattleResults(attackers, defenders, m_target, m_data, DSettings.LoadSettings().CA_CM_determinesAttackResultsToSeeIfTaskWorthwhile, true);
		final List<Unit> responseAttackers = DUtils.DetermineResponseAttackers(m_data, GlobalCenter.CurrentPlayer, m_target, simulatedAttack);
		final List<Unit> responseDefenders = Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air can't defend ter because they need to land
		...
		if (m_taskType == CM_TaskType.Land_Attack_Offensive)
		{
			...
			if (howCloseToMeetingMinSurvivalChance < .98F)
			{
				...
				simulatedResponse = DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target, m_data, DSettings.LoadSettings().CA_CMNCM_determinesResponseResultsToSeeIfTaskWorthwhile,
							false);
				final int tradeScoreIfAttacked = -DUtils.GetTaskTradeScore(m_data, m_target, responseAttackers, responseDefenders, simulatedResponse, new ArrayList<Unit>(), new ArrayList<Unit>(),
							null);
		...
		else
		{
			...
			final Territory startTer = TacticalCenter.get(m_data, player).GetUnitLocationAtStartOfTurn(m_recruitedUnits.get(0).GetFirstUnit()); // Land grabs are done with only one unit
			...
			final float chanceOfHittingCheapestLAttacker = UnitAttachment.get(GetRecruitedUnitsAsUnitList().get(0).getUnitType()).getDefense(player) / 6.0F;
