...
public class CM_Task
{
    ...
    public boolean IsPlannedAttackWorthwhile(List<CM_Task> allTasks)
    {
        ...
        if (m_target.getOwner().isNull())
        {
            ...
            if (DUtils.GetTerritoriesWithinXDistanceOfYMatchingZAndHavingRouteMatchingA(m_data, m_target, Integer.MAX_VALUE, DMatches.territoryIsOwnedByNNEnemy(m_data, player), Matches.TerritoryIsLand).size() >= 3)
            {
        ...
        if (m_taskType == CM_TaskType.Attack_Offensive || m_taskType == CM_TaskType.Attack_Trade)
        {
            ...
            if (closestToTargetEnemyCap != null)
            {
                ...
                if (!targetIsTooCloseToCap && strategyType == StrategyType.Enemy_Defensive && targetIsCloserToCapThanFromLocations)
                {
                    ...
                    List<Unit> ourUnitsInArea = DUtils.GetUnitsMatchingXInTerritories(DUtils.GetTerritoriesWithinXDistanceOfYMatchingZAndHavingRouteMatchingA(m_data, m_target, 2, Matches.TerritoryIsLand, Matches.TerritoryIsLand), Matches.unitIsOwnedBy(player));
