...
public class OCBattle {
  ...
  public void rollBattle() {
    ...
    if (supportUnits != null && supportUnits.length > 0) {
      int supportHits = 0;
      for (final Integer supportUnit : supportUnits) {
        final UnitGroup curG = locAttackers.elementAt(supportUnit.intValue());
        supportHits += curG.rollUnitsAttack();
        locAttackers.removeElementAt(supportUnit.intValue());
      }
    ...
    for (int i = 0; i < unitSize; i++) {
      if (attackers) {
        if (units.elementAt(i).getUnit().getAttackValue() < lowest) {
          if (getNumberOfLand(units) == 1 && this.preserveLand && getNumberOfUnits(units) != 1
