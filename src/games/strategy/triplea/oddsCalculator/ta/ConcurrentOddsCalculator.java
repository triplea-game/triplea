...
public class ConcurrentOddsCalculator implements IOddsCalculator {
  ...
  private volatile int m_cancelCurrentOperation = 0; // shortcut setting of previous game data if we are trying to set
                                                     // it to a new one, or
                                                     // shutdown
  private final CountUpAndDownLatch m_latchSetData = new CountUpAndDownLatch(); // do not let calcing happen while we
                                                                                // are setting game data
  private final CountUpAndDownLatch m_latchWorkerThreadsCreation = new CountUpAndDownLatch(); // do not let setting of
                                                                                              // game data happen
  ...
  private final Object m_mutexCalcIsRunning = new Object(); // do not let multiple calculations or setting calc data
                                                            // happen at same time
  ...
  public void setGameData(final GameData data) {
    ...
    synchronized (m_mutexSetGameData) {
      try {
        m_latchWorkerThreadsCreation.await(); // since setting data takes place on a different thread, this is our
                                              // token. wait on it since
      ...
      if (data == null || m_isShutDown) {
        ...
        ++m_cancelCurrentOperation;
        m_latchWorkerThreadsCreation.increment();// increment our token, so that we can set the data in a different
                                                 // thread and return from
                                                 // this one
  ...
  private static int getThreadsToUse(final long timeToCopyInMillis, final long memoryUsedBeforeCopy) { // use both time
                                                                                                       // and memory
                                                                                                       // left to
                                                                                                       // determine how
                                                                                                       // many copies to
    ...
    final long memoryLeftBeforeMax = runtime.maxMemory() - (Math.max(usedMemoryAfterCopy, memoryUsedBeforeCopy)); // we
                                                                                                                  // can
                                                                                                                  // not
                                                                                                                  // predict
                                                                                                                  // how
                                                                                                                  ...
                                                                                                                  // gc
                                                                                                                  // works
    final long memoryUsedByCopy = Math.max(100000, (usedMemoryAfterCopy - memoryUsedBeforeCopy)); // make sure it is a
                                                                                                  // decent size
                                                                                                  // regardless of how
                                                                                                  // stupid the gc is
    ...
    if (timeToCopyInMillis > 3000) {
      return Math.min(numberOfTimesWeCanCopyMax, Math.max(1, (MAX_THREADS / 2))); // use half the number of threads
                                                                                  // available if we took
  ...
  private void createWorkers(final GameData data) {
      ...
      try {
        ...
        { // if 2 or fewer threads, do not multi-thread the copying (we have already copied it once above, so at most
          ...
          while (m_cancelCurrentOperation >= 0 && i < m_currentThreads) {
            m_workers.add(new OddsCalculator(newData, (m_currentThreads == ++i))); // the last one will use our already
                                                                                   // copied data from
          ...
          m_workers.add(new OddsCalculator(newData, true)); // the last one will use our already copied data from above,
                                                            // without copying it
                                                            // again
          try {
            workerLatch.await();
    ...
    if (m_cancelCurrentOperation < 0 || data == null) {
      ...
      m_isDataSet = true;// should make sure that all workers have their game data set before we can call calculate and
                         // other things
  ...
  private void awaitLatch() {
    try {
      m_latchSetData.await();// there is a small chance calculate or setCalculateData or something could be called in
                             // between calls to
                             // setGameData
    } catch (final InterruptedException e) {
    }
    ...
    synchronized (m_mutexCalcIsRunning) {
      ...
      for (final OddsCalculator worker : m_workers) {
        if (!m_isDataSet || m_isShutDown) {
          return;// we could have attempted to set a new game data, while the old one was still being set, causing it to
                 // abort with null
                 // data
        }
  ...
  public AggregateResults calculate() throws IllegalStateException {
    synchronized (m_mutexCalcIsRunning) {
      ...
      for (final OddsCalculator worker : m_workers) {
        if (!getIsReady()) {
          return new AggregateResults(0);// we could have attempted to set a new game data, while the old one was still
                                         // being set, causing
