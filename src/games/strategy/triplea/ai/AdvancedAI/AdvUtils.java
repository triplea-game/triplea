...
public class AdvUtils
{
	...
	/**
	 * Returns the order of players starting with the current player
	 * 
	 * @param data
	 * @param player
	 * @return
	 */
	public static Set<PlayerID> relativePlayerOrder(final GameData data, final PlayerID player)
	{
		final Set<PlayerID> orderedPlayers = new LinkedHashSet<PlayerID>();
		int playerPosition = 0;
		orderedPlayers.add(player);
		boolean foundPlayer = false;
		final PlayerList allPlayers = data.getPlayerList();
		final Iterator<PlayerID> pIter = allPlayers.iterator();
		while (pIter.hasNext())
		{
			final PlayerID thisPlayer = pIter.next();
			...
			if (foundPlayer)
				orderedPlayers.add(thisPlayer);
			else
				playerPosition++;
		}
		final Iterator<PlayerID> pIter2 = allPlayers.iterator();
		for (int i = 0; i < playerPosition; i++)
		{
			orderedPlayers.add(pIter2.next());
		}
		return orderedPlayers;
	}
	
	/**
	 * Determine the amount of cash available for a player
	 * 
	 * @param data
	 * @param player
	 * @return
	 */
	public static int getPUsAvailable(final GameData data, final PlayerID player)
	{
		final int ownerCash = player.getResources().getQuantity(data.getResourceList().getResource(Constants.PUS));
		return ownerCash;
	}
	
	public static ProductionRule getProductionRuleByUnitType(final Collection<ProductionRule> allRules, final UnitType unitT)
	{
		for (final ProductionRule aRule : allRules)
		{
			final UnitType type = (UnitType) aRule.getResults().keySet().iterator().next();
			if (unitT.getName().equals(type.getName()))
				return aRule;
		}
		return null;
	}
	...
	/**
	 * Determine the maximum # of units that can be purchased for a given amount of cash
	 * 
	 * @param ProductionRule
	 * @param GameData
	 * @param unit
	 * @param ownerCash
	 * @return Array of int containing maxPurchase and unitCost
	 */
	
	public static int[] maxUnitPlayerCanPurchase(final ProductionRule aRule, final GameData data, final UnitType unit, final int ownerCash)
	{
		int maxUnits = 0, unitCost = 0;
		final int[] unitDetails = { maxUnits, unitCost };
		final Resource pus = data.getResourceList().getResource(Constants.PUS);
		if (aRule == null)
			return unitDetails;
		unitCost = aRule.getCosts().getInt(pus);
		if (unitCost > 0)
			maxUnits = ownerCash / unitCost;
		unitDetails[0] = maxUnits;
		unitDetails[1] = unitCost;
		return unitDetails;
	}
	...
	public static boolean threatToAlliedCapitals(final GameData data, final PlayerID player, final List<Territory> threats, final boolean tFirst)
	{
		...
		final Set<PlayerID> oPlayers = relativePlayerOrder(data, player);
		for (final PlayerID otherPlayer : oPlayers)
		{
		...
		{
			final float landThreat = getStrengthOfPotentialAttackers(cap, data, player, tFirst, true, null, primaryAttacker);
			final PlayerID thisCapBelongsTo = cap.getOwner();
			final int ownerCash = getPUsAvailable(data, thisCapBelongsTo);
			final int totalProduction = TerritoryAttachment.get(cap).getUnitProduction();
			final float additionalStrengthOnHisTurn = Math.min((ownerCash) / 2.0F, (totalProduction) * 2.0F);
			float capStrength = strength(cap.getUnits().getMatches(Matches.alliedUnit(player, data)), false, false, tFirst) + 5.0F;
			final Iterator<PlayerID> pIter = oPlayers.iterator();
			int capP = 0, enemyP = 0;
			final int position = 0;
			while (pIter.hasNext())
			{
				final PlayerID tPlayer = pIter.next();
				if (tPlayer.equals(thisCapBelongsTo))
					capP = position;
				else if (tPlayer.equals(primaryAttacker))
					enemyP = position;
			}
			capStrength += (capP < enemyP) ? additionalStrengthOnHisTurn : 0.0F;
	 ...
	 * @return
	 */
	public static Territory landAttackMap(final GameData data, final PlayerID player, final HashMap<Territory, Float> enemyMap, final HashMap<Territory, Integer> AIStratMap)
	{
		Territory largestTerr = null;
		final List<Territory> enemyTerrs = AdvUtils.allEnemyTerritories(data, player);
		...
		while (eTIter.hasNext())
		{
			...
			{
				float eStrength = AdvUtils.strength(eTerr.getUnits().getMatches(Matches.enemyUnit(player, data)), true, false, true);
				if (AIStratMap != null)
					eStrength += (AIStratMap.containsKey(eTerr) ? AIStratMap.get(eTerr) : 0.0);
				enemyMap.put(eTerr, eStrength);
			}
		}
		AdvUtils.reorder(enemyTerrs, enemyMap, true);
		if (enemyTerrs.isEmpty())
		{
			return null;
		}
	...
	public static void continentAlliedUnitTerr(final GameData data, final PlayerID player, final Territory startTerr, final List<Territory> contiguousTerr, final List<Territory> ignoreTerr)
	{
		...
		for (final Territory n1 : neighbor1)
		{
			if (Matches.isTerritoryAllied(player, data).match(n1))
				...
				ignoreTerr.add(n1);
			AdvUtils.continentAlliedUnitTerr(data, player, n1, contiguousTerr, ignoreTerr);
		}
	...
	public static float strengthOnContinent(final GameData data, final PlayerID player, final Territory continentTerr)
	{
		...
		final boolean island = !AdvUtils.doesLandExistAt(continentTerr, data, false); // just make sure this is really a "continent"
		if (island)
		{
			if (Matches.isTerritoryAllied(player, data).match(continentTerr))
			{
				continentStrength += AdvUtils.strength(continentTerr.getUnits().getMatches(Matches.alliedUnit(player, data)), true, false, false);
		...
		AdvUtils.continentAlliedUnitTerr(data, player, continentTerr, allContinentTerr, ignoreTerr);
		for (final Territory cTerr : allContinentTerr)
			continentStrength += AdvUtils.strength(cTerr.getUnits().getMatches(Matches.alliedUnit(player, data)), true, false, false);
		return continentStrength;
	}
	...
	public static void removeUnthreatenedTerritories(final GameData data, final PlayerID player, final List<Territory> terrList)
	{
		...
		while (tIter.hasNext())
		{
			...
			{
				final float eStrength = AdvUtils.getStrengthOfPotentialAttackers(checkTerr, data, player, false, false, null, primaryAttacker);
	...
	public static IntegerMap<Territory> targetTerritories(final GameData data, final PlayerID player, final int tDistance)
	{
		...
		final List<Territory> playerFactories = AdvUtils.findUnitTerr(data, player, enemyFactory);
		...
		for (final Territory mT : playerFactories)
		{
			...
			for (final Territory checkTerr : initialGroup)
			{
				if (Matches.isTerritoryEnemy(player, data).match(checkTerr) && Matches.TerritoryIsLand.match(checkTerr) && Matches.TerritoryIsNotImpassable.match(checkTerr))
				{
					...
					if (cDist < checkDist || (cDist >= checkDist && !AdvUtils.doesLandExistAt(checkTerr, data, false)))
					{
				...
				final HashMap<Integer, IntegerMap<Territory>> unitCountList, final PlayerID player, final GameData data, final boolean tFirst)
	{
		...
		int bestFit = -1;
		float maxScore = 0.0F;
		final List<Territory> enemyCaps = AdvUtils.getEnemyCapitals(data, player);
		final List<Territory> enemyFactories = AdvUtils.findUnitTerr(data, player, enemyFactory);
		/*
		final List<Territory> ourFriendlyTerr = new ArrayList<Territory>();
		final List<Territory> ourEnemyTerr = new ArrayList<Territory>();
		final HashMap<Territory, Float> rankMap = AdvUtils.rankTerritories(data, ourFriendlyTerr, ourEnemyTerr, null, player, tFirst, false);
		*/
		final List<Territory> allEnemyTerr = AdvUtils.allEnemyTerritories(data, player);
		...
		for (Integer i = 0; i <= numMoves - 1; i++)
		{
			...
			for (final Territory eTerr : allEnemyTerr)
			{
				...
				{
					eNeighbors.addAll(AdvUtils.getExactNeighbors(eTerr, j, player, data, false));
	...
	public static IntegerMap<PlayerID> getPlayerTUV(final GameData data)
	{
		...
		for (final Territory allTerr : data.getMap().getTerritories())
		{
			for (final PlayerID onePlayer : playerList)
			{
				...
				final IntegerMap<UnitType> uMap = AdvUtils.convertListToMap(playerUnits);
				int tuv = TUVMap.getInt(onePlayer);
				tuv += AdvUtils.determineTUV(uMap, costMap.get(onePlayer));
				...
				final PlayerID player, final boolean tFirst)
	{
		...
		final List<Territory> ignoreTerr = new ArrayList<Territory>();
		AdvUtils.continentAlliedUnitTerr(data, player, targetTerr, contiguousTerritories, ignoreTerr);
		...
		AdvUtils.reorder(contiguousTerritories, distanceMap, false);
		AdvUtils.trimTerritoryList(contiguousTerritories, 3); // look at the top 3
		...
		if (goTerr == null)
		{
			AdvUtils.reorder(contiguousTerritories, unitMap, true);
			if (!contiguousTerritories.isEmpty())
	...
	{
		final List<Territory> eCapitols = AdvUtils.getEnemyCapitals(data, player);
		int maxDistance = 100;
				...
				final HashMap<PlayerID, IntegerMap<UnitType>> costMap, final PlayerID player, final GameData data, final boolean aggressive, final boolean subRestricted, final boolean tFirst)
	{
		...
		if (weWin && (eTUV == 0 || (((AdvUtils.strength(defenderUnits, false, eTerr.isWater(), tFirst) * 5F) + 10F) < AdvUtils.strength(invasionUnits, true, eTerr.isWater(), tFirst))))
	...
	public static List<Territory> getTerritoriesWithEnemyNeighbor(final GameData data, final PlayerID player, final boolean allied, final boolean neutral)
	{
		...
		final List<Territory> enemyLandTerr = AdvUtils.allEnemyTerritories(data, player);
		if (!neutral)
		{
		...
		while (eIter.hasNext())
		{
			...
			if (AdvUtils.doesLandExistAt(enemy, data, false))
			{
				final List<Territory> newTerrs = new ArrayList<Territory>();
				if (allied)
					newTerrs.addAll(AdvUtils.getNeighboringLandTerritories(data, player, enemy));
				else
				...
				final boolean tFirst)
	{
		...
		{
			eStrength = getStrengthOfPotentialAttackers(xWaterTerr, data, player, tFirst, false, ignoreTerr, primaryAttacker);
			...
			eStrength -= AdvUtils.strength(xWaterTerr.getUnits().getMatches(alliedSeaUnit), false, true, tFirst);
	...
	public static List<Territory> possibleBlitzTerritories(final Territory checkTerr, final GameData data, final PlayerID player)
	{
		...
		final List<Territory> twoTerr = AdvUtils.getExactNeighbors(checkTerr, 2, player, data, false);
		final List<Territory> oneTerr = AdvUtils.getExactNeighbors(checkTerr, 1, player, data, false);
		for (final Territory blitzFrom : twoTerr)
		{
			...
			if (blitzUnits.isEmpty())
				continue;
			final List<Territory> blitzNeighbors = AdvUtils.getExactNeighbors(blitzFrom, 1, player, data, false);
	 ...
	 */
	/*
	public static int distanceToEnemyOld(final Territory t, final List<Territory> beenThere, final GameData data, final PlayerID player, final boolean sea)
	{ // find the distance to the closest enemy land territory by recursion
		// if no enemy territory can be found...it returns 0
		if (Matches.TerritoryIsImpassable.match(t))
			return 0;
		final CompositeMatch<Territory> enemyWater = new CompositeMatchAnd<Territory>(Matches.TerritoryIsWater, Matches.territoryHasEnemyUnits(player, data));
		final CompositeMatch<Territory> noEnemyAndWater = new CompositeMatchAnd<Territory>(Matches.TerritoryIsWater, Matches.territoryHasNoEnemyUnits(player, data));
		final List<Territory> thisTerr = new ArrayList<Territory>();
		if (sea)
		...
		{
			final List<Territory> newTerrList = new ArrayList<Territory>();
			if (sea)
			...
			{
				int minDist = 100;
				for (final Territory t2 : newTerrList)
				{
					final int aDist = distanceToEnemy(t2, new ArrayList<Territory>(beenThere), data, player, sea);
	...
	*/
	...
	public static float getStrengthOfPotentialAttackers(final Territory location, final GameData data, final PlayerID player, final boolean tFirst, final boolean ignoreOnlyPlanes,
				final List<Territory> ignoreTerr, PlayerID primaryAttacker)
	{
		...
		while (playerIter.hasNext())
		{
			...
			// final List<Territory> eFTerrs = AdvUtils.findUnitTerr(data, ePlayer, enemyPlane);
			...
			airStrength += AdvUtils.invitePlaneAttack(false, false, location, 1000.0F, null, null, null, data, ePlayer);
			/*
			...
			if (maxBomberDistance < 0)
				maxBomberDistance = 0;
			*/
			final List<Territory> eTTerrs = AdvUtils.findUnitTerr(data, ePlayer, aTransport);
			...
			{ // old assumed fleets won't split up, new lets them. no biggie.
				...
				for (int i = 2; i <= 3; i++)
				{
					final List<Territory> moreTerr = getExactNeighbors(location, i, player, false);
					if (doIgnoreTerritories)
						moreTerr.removeAll(ignoreTerr);
					for (final Territory shipTerr : moreTerr)
					{
						if (!shipTerr.isWater())
							continue;
						else if (ePlayer == null)
							continue;
						final List<Unit> moreEnemies = shipTerr.getUnits().getMatches(enemyShip);
						if (moreEnemies.isEmpty())
							continue;
						final int maxShipDistance = MoveValidator.getLeastMovement(moreEnemies);
						final Route seaRoute = getMaxSeaRoute(data, shipTerr, location, ePlayer, true, maxShipDistance);
				...
				for (final Territory t4 : data.getMap().getNeighbors(location, maxTransportDistance))
				{
					...
					while (!transportsCounted && iterTerr.hasNext())
					{
						...
						for (final Territory xN : transNeighbors)
						{
							...
							final List<Unit> availTransUnits = AdvUtils.sortTransportUnits(aTransUnits);
				...
				final List<Territory> checked)
	{
		...
		while (!q.isEmpty())
		{
			...
			if (distance.getInt(current) == maxDistance)
				break;
			for (final Territory neighbor : data.getMap().getNeighbors(current, AdvUtils.TerritoryIsNotImpassableToAirUnits(data)))
			{
				if (!distance.keySet().contains(neighbor))
				{
					q.add(neighbor);
	 ...
	 */
	/*
	public static Route findNearestOld(final Territory start, final Match<Territory> endCondition, final Match<Territory> routeCondition, final GameData data)
	{
	...
	*/
				...
				final int maxUnits, final GameData data)
	{
		...
		for (final Territory t : data.getMap().getTerritories())
		{
			if (endCondition.match(t))
			{
				final List<Unit> countUnits = t.getUnits().getMatches(unitCondition);
	...
	public static Route findNearestNotEmpty(final Territory start, final Match<Territory> endCondition, final Match<Territory> routeCondition, final GameData data)
	{
		...
		else
		{
			for (final Territory t2 : data.getMap().getTerritories())
			{
				...
				{
					final CompositeMatchOr<Territory> routeOrEnd = new CompositeMatchOr<Territory>(routeCondition, Matches.territoryIs(t2));
					
					final Route r = data.getMap().getRoute(start, t2, routeOrEnd);
					if (r != null)
					{
						if (shortestRoute == null || r.getLength() < shortestRoute.getLength())
							shortestRoute = r;
	...
	public static boolean airUnitIsLandableOnCarrier(final Unit u, final Territory source, final Territory target, final PlayerID player, final GameData data)
	{
		...
		{
			final Route r = findNearest(target, condition, AdvUtils.TerritoryIsNotImpassableToAirUnits(data), data);
			if (r == null)
				return false;
	...
	public static Territory findFactoryTerritory(final GameData data, final PlayerID player, final float risk, boolean buyfactory, final boolean onWater)
	{
		...
		final List<Territory> owned = allOurTerritories(data, player);
		final List<Territory> existingFactories = AdvUtils.findTersWithUnitsMatching(data, player, Matches.UnitCanProduceUnits); // TODO: maybe use Matches.UnitCanProduceUnitsAndIsConstruction
		...
		if (onWater)
		{
			final List<Territory> waterOwned = AdvUtils.stripLandLockedTerr(data, owned);
			...
			AdvUtils.reorder(owned, terrProd, true);
		...
		for (final Territory t : owned)
		{
			...
			for (final Territory factTerr : existingFactories)
			{
				final Route fRoute = data.getMap().getRoute(t, factTerr, Matches.TerritoryIsNotImpassableToLandUnits(player));
			...
			*/
			...
			final List<Territory> closeAllies = AdvUtils.getNeighboringLandTerritories(data, player, t);
			...
			{
				tStrength += AdvUtils.strength(cA.getUnits().getMatches(Matches.alliedUnit(player, data)), false, false, false);
			}
			...
			*/
			...
			if ((twoCheckStrength + threeCheckStrength * 0.5) > (puValue * 8.0F + tStrength) * 4) // take at least 2 moves to invade (veqryn multiplying friendly for now)
			{
				badIdea = true;
			}
			if (badIdea)
	...
	public static List<Territory> getExactNeighbors(final Territory territory, final int distance, final PlayerID player, final GameData data, final boolean neutral)
	{
		...
		final List<Territory> startClone = new ArrayList<Territory>();
		if (distance == 0)
			return startClone;
		...
		if (!neutral || Properties.getNeutralsImpassable(data))
		{
			final Iterator<Territory> t = startClone.iterator();
			while (t.hasNext())
			{
		...
		final Iterator<Territory> t2 = startClone.iterator();
		while (t2.hasNext())
		{
		...
		final List<Territory> startX = new ArrayList<Territory>();
		final List<Territory> innerCircle = new ArrayList<Territory>();
		final Set<Territory> start = data.getMap().getNeighbors(territory);
		...
		if (distance > 1)
		{
			...
			for (int i = 2; i <= distance; i++)
			{
				...
				startX.clear();
				final Set<Territory> start2 = data.getMap().getNeighbors(territory, i);
	...
	public static Territory findASeaTerritoryToPlaceOn(final Territory landTerr, final GameData data, final PlayerID player, final boolean tFirst)
	{
		...
		{
			float enemyStrength = getStrengthOfPotentialAttackers(t, data, player, tFirst, true, null, primaryAttacker);
			final float extraEnemy = AdvUtils.strength(t.getUnits().getMatches(Matches.enemyUnit(player, data)), true, true, tFirst);
		...
		if (seaPlaceAt == null && bestSeaPlaceAt == null)
		{
			...
			{
				final float enemyStrength = getStrengthOfPotentialAttackers(t, data, player, tFirst, true, null, primaryAttacker);
				...
				final List<Route> moveRoutes, final GameData data, final PlayerID player, final boolean attacking, final boolean tFirst, final boolean includeTransports, final Match<Unit> types)
	{
		...
		final List<Territory> possibleShipTerr = AdvUtils.findTersWithUnitsMatching(data, player, ownedSeaUnit);
				...
				final List<Territory> seaTerrAttacked)
	{
		...
		{
			final float strengthAtTarget = getStrengthOfPotentialAttackers(waterCheck, data, player, tFirst, false, seaTerrAttacked, primaryAttacker);
	...
	public static float invitePlaneAttack(final boolean noncombat, final boolean fightersOnly, final Territory target, float remainingStrengthNeeded, Collection<Unit> unitsAlreadyMoved,
				final List<Collection<Unit>> moveUnits, final List<Route> moveRoutes, final GameData data, final PlayerID player)
	{
		...
		boolean saveUnitMovements = true;
		if (moveUnits == null || moveRoutes == null)
			saveUnitMovements = false;
		...
		if (unitsAlreadyMoved == null)
		{
			unitsAlreadyMoved = new ArrayList<Unit>();
		}
		if (isWater)
		{
			if (noncombat)
			{
				for (final Unit ac : ACUnits)
				{
					availSpace += UnitAttachment.get(ac.getUnitType()).getCarrierCapacity();
					for (final Unit f : fightersOnAC)
					{
						availSpace -= UnitAttachment.get(f.getUnitType()).getCarrierCost();
					}
				}
			}
			for (final Territory owned : planeOnWater) // make sure that these planes are not already involved in an attack
			{
				...
				if (remainingStrengthNeeded > committedStrength && (!Matches.territoryHasEnemyUnits(player, data).match(owned) || noncombat))
				{
					...
					if (saveUnitMovements && !tmpUnits2.isEmpty())
					{
		...
		for (final Territory owned : planeTerr) // make sure that these planes are not already involved in an attack
		{
			...
			if (remainingStrengthNeeded > committedStrength && !Matches.territoryHasEnemyUnits(player, data).match(owned))
			{
				...
				if (thisRoute == null)
					continue;
				// final int rDist = thisRoute.numberOfSteps();
				...
				// if (unitsAlreadyMoved != null)
				allAirUnits.removeAll(unitsAlreadyMoved);
				if (allAirUnits.isEmpty())
					continue;
				for (final Unit u2 : allAirUnits)
				{
					final int movementRemaining = thisRoute.getMovementLeft(u2);
	 ...
	 * 
	 * 07/2012
	 * Adequately addresses varied movements of blitzers and should move units with large movement
	 */
	...
	{// Blitz through owned into enemy
		...
		final Collection<Territory> availableTerrs = allAlliedTerritories(data, player);
		final Collection<Territory> terrClone = new ArrayList<Territory>(availableTerrs);
		for (final Territory armorTerr : terrClone)
		{
			if (!Matches.territoryHasLandUnitsOwnedBy(player).match(armorTerr) || Matches.TerritoryIsWater.match(armorTerr))
			{
				availableTerrs.remove(armorTerr);
			}
			else if (!Matches.territoryHasUnitsThatMatch(blitzUnit).match(armorTerr))
			{
				availableTerrs.remove(armorTerr);
			}
		}
		final List<Territory> blitzFrom = new ArrayList<Territory>(availableTerrs);
		final List<Territory> blitzCopy = new ArrayList<Territory>(blitzFrom);
		Route tRoute = null;
		float totStrength = 0.0F;
		for (final Territory t : blitzCopy)
		{
			final Collection<Unit> blitzUnits = t.getUnits().getMatches(blitzUnit);
			blitzUnits.removeAll(unitsAlreadyMoved);
			if (blitzUnits.isEmpty())
				continue;
			final int maxStep = MoveValidator.getMaxMovement(blitzUnits);
			if (nonCombat)
				// tRoute = getTwoRoute(t, enemy, alliedAndNotWater, null, data);
				tRoute = getMaxStepRoute(t, enemy, maxStep, alliedAndNotWater, null, data);
			else
				tRoute = getMaxStepRoute(t, enemy, maxStep, noEnemyUnitsAndNotWater, null, data);
			if (tRoute == null)
				blitzFrom.remove(t);
		}
		final List<Unit> blitzUnits = new ArrayList<Unit>();
		float bStrength = 0.0F;
		if (forced && !nonCombat) // if a route is available, bring in the units no matter what
		{
			for (final Territory blitzTerr : blitzFrom)
			{
				...
				final Route blitzRoute = getMaxStepRoute(blitzTerr, enemy, maxStep, noEnemyUnitsAndNotWater, null, data);
				if (blitzRoute != null)
				{
					for (final Unit blitzer : blitzUnits)
					{
						final int moveRemaining = blitzRoute.getMovementLeft(blitzer);
						if (moveRemaining >= 0 && (remainingStrengthNeeded > totStrength))
						{
	...
	/**
	 * Make a route in which the condition only applies to the middle territories in the route and not the target
	 * 
	 * @param blockedTerr
	 *            - List of Territories the route cannot pass through
	 */
	public static Route getMaxStepRoute(final Territory t1, final Territory t2, final int maxSteps, final Match<Territory> condition, final List<Territory> blockedTerr, final GameData data)
	{
		final Collection<Territory> t1NeighborCollection = data.getMap().getNeighbors(t1);
		if (t1.equals(t2) || data.getMap().getLandRoute(t1, t2) == null || t1NeighborCollection.contains(t2))
			return null;
		final Route newRoute = data.getMap().getRoute_IgnoreEnd(t1, t2, condition);
		if (newRoute == null)
			return null;
		if (newRoute.numberOfSteps() <= maxSteps) // this is fine
			return newRoute;
		return null;
	}
	
	/**
	 * Quick and Dirty strip out any territory which cannot
	 * generate a valid Land Route to the target
	 * 
	 * @param data
	 * @param targetTerr
	 * @param checkTerrs
	 */
	public static void removeTerrsWithNoValidLandRoute(final GameData data, final Territory targetTerr, final Collection<Territory> checkTerrs)
	{
		final Collection<Territory> tClone = new ArrayList<Territory>(checkTerrs);
		for (final Territory t : tClone)
		{
			final Route testRoute = data.getMap().getLandRoute(t, targetTerr);
			if (testRoute == null)
				checkTerrs.remove(t);
		}
	}
	...
	/**
	 * Determine the enemy potential for blitzing a territory (rewritten 07/2012)
	 * 
	 * @param blitzHere
	...
	public static float determineEnemyBlitzStrength(final Territory blitzHere, final List<Route> blitzTerrRoutes, final List<Territory> blockTerr, final GameData data, final PlayerID ePlayer)
	{
		...
		final Collection<Territory> eTerrs = data.getMap().getTerritoriesOwnedBy(ePlayer);
		removeTerrsWithNoValidLandRoute(data, blitzHere, eTerrs);
		float bStrength = 0.0F;
		final Collection<Unit> blitzUnits = new ArrayList<Unit>();
		for (final Territory blitzTerr : eTerrs)
		{
			blitzUnits.clear();
			// final List<Unit> tmpBlitz = new ArrayList<Unit>();
			blitzUnits.addAll(blitzTerr.getUnits().getMatches(blitzUnit));
			...
			final int maxStep = MoveValidator.getMaxMovement(blitzUnits);
			// final Route blitzRoute = getTwoRoute(blitzTerr, enemy, noEnemyUnitsAndNotWater, null, data);
			final Route blitzRoute = getMaxStepRoute(blitzTerr, blitzHere, maxStep, validBlitzRoute, null, data);
			if (blitzRoute != null)
			{
				for (final Unit blitzer : blitzUnits)
				{
					...
					if (moveRemaining >= 0)
					{
						bStrength += uStrength(blitzer, true, false, false);
					}
				}
			}
		}
		...
		float eStrength = 0.0F;
		final List<Territory> pBlitzTerr = getExactNeighbors(blitzHere, 2, ePlayer, false);
		final Set<Territory> blitzStepTerr = data.getMap().getNeighbors(blitzHere, validBlitzRoute);
		final Iterator<Territory> bIter = pBlitzTerr.iterator();
		while (bIter.hasNext())
		{
			final Territory checkTerr = bIter.next();
			if (checkTerr.isWater() || Matches.TerritoryIsImpassableToLandUnits(ePlayer).match(checkTerr))
				bIter.remove();
		}
		if (blitzStepTerr.isEmpty())
			return 0.0F;
		for (final Territory checkBlitzTerr : pBlitzTerr)
		{
			final Route blitzRoute = getTwoRoute(checkBlitzTerr, blitzHere, validBlitzRoute, blockTerr, data);
			if (blitzRoute == null)
				continue;
			final List<Unit> blitzUnits = checkBlitzTerr.getUnits().getMatches(blitzUnit);
			if (blitzUnits.isEmpty())
				continue;
	...
	{
		final boolean canMove = Match.allMatch(moveUnits, Matches.UnitHasEnoughMovementForRoute(moveRoute));
		if (!canMove)
		{
			...
			while (routeIter.hasNext() && !routeDone)
			{
				...
				if (Match.allMatch(moveUnits, Matches.UnitHasEnoughMovementForRoute(newRoute)))
				{
	...
	public static float strengthOfTerritory(final GameData data, final Territory thisTerr, final PlayerID player, final boolean attacking, final boolean sea, final boolean tFirst, final boolean allied)
	{
		...
		final float theStrength = AdvUtils.strength(theUnits, attacking, sea, tFirst);
		return theStrength;
	}
	...
	public static float verifyPlaneAttack(final GameData data, final List<Collection<Unit>> xMoveUnits, final List<Route> xMoveRoutes, final PlayerID player, final List<Territory> alreadyAttacked)
	{
		...
		for (final Territory alliedTerr : AdvUtils.allAlliedTerritories(data, player))
		{
			final float eStrength = AdvUtils.getStrengthOfPotentialAttackers(alliedTerr, data, player, false, false, alreadyAttacked, primaryAttacker);
			float ourStrength = AdvUtils.strengthOfTerritory(data, alliedTerr, player, false, false, false, true);
		...
		while (xMoveIter.hasNext())
		{
			...
			while (xMIter.hasNext())
			{
				...
				if (Matches.UnitIsAir.match(plane))
				{
					...
					AdvUtils.reorder(endNeighbors, strengthDiffMap, false);
					...
					while (eIter.hasNext() && !safePlane)
					{
						...
						{
							final float strengthDiff = strengthDiffMap.get(newTerr) - AdvUtils.uStrength(plane, false, false, false);
		...
		for (final Territory checkTerr : badMoveTerrs)
		{
			...
			for (final Integer killRoute : routeNumber)
			{
				...
				strengthEliminated += AdvUtils.strength(killUnits, true, false, false);
				...
				final HashMap<ProductionRule, Boolean> supportableInfMap, final GameData data, final PlayerID player, final int fighters)
	{
		...
		for (int i = 0; i <= (usableMaxUnits - totUnits); i++)
		{
			...
			/*
			final Iterator<ProductionRule> intCheckIter = ruleCheck.iterator();
	...
	public static Route getMaxSeaRoute(final GameData data, final Territory start, final Territory destination, final PlayerID player, final boolean attacking, final int maxDistance)
	{
		...
		{
			final Territory currentTerr = r.getTerritories().get(i);
			if (Matches.territoryHasEnemyUnits(player, data).match(currentTerr))
			{
				final Territory previousTerr = r.getTerritories().get(i - 1);
				final Set<Territory> currNeighbors = data.getMap().getNeighbors(currentTerr, routeCond);
				final Territory nextTerr = r.getTerritories().get(i + 1);
				currNeighbors.remove(previousTerr);
				currNeighbors.remove(nextTerr);
				boolean switchTerr = false;
				for (final Territory xTerr : currNeighbors)
				{
					if (switchTerr)
						continue;
					final Set<Territory> xNeighbors = data.getMap().getNeighbors(xTerr);
					if (xNeighbors.contains(previousTerr) && xNeighbors.contains(nextTerr))
					{
						final Route newRoute = new Route();
						final List<Territory> oldRTerrs = r.getTerritories();
	...
	public static boolean territoryHasThreatenedAlliedFactoryNeighbor(final GameData data, final Territory eTerr, final PlayerID player)
	{
		...
		{
			float eStrength = AdvUtils.getStrengthOfPotentialAttackers(factory, data, player, false, true, null, primaryAttacker);
			eStrength += eStrength * 1.15F + (eStrength > 2.0F ? 3.0F : 0.0F);
			float myStrength = AdvUtils.strength(factory.getUnits().getUnits(), false, false, false);
			if (eStrength > myStrength)
			{
				...
				{
					addStrength += AdvUtils.strengthOfTerritory(data, fNTerr, player, false, false, false, true);
				}
	...
	public static HashMap<Territory, Float> rankTerritories(final GameData data, final List<Territory> ourFriendlyTerr, final List<Territory> ourEnemyTerr, final List<Territory> ignoreTerr,
				final HashMap<Territory, Integer> terrStrat, final PlayerID player, final boolean tFirst, final boolean waterBased, final boolean nonCombat)
	{
		long last, now, start;
		 ...
		 */
		final List<Territory> alliedFactories = AdvUtils.getEnemyCapitals(data, ePlayer);
		final Iterator<Territory> aFIter = alliedFactories.iterator();
		while (aFIter.hasNext())
		{
			final Territory aFTerr = aFIter.next();
			final float aFPotential = AdvUtils.getStrengthOfPotentialAttackers(aFTerr, data, player, tFirst, true, null, primaryAttacker);
			final float alliedStrength = AdvUtils.strengthOfTerritory(data, aFTerr, player, false, false, tFirst, true);
		...
		for (final Territory eTerr : data.getMap().getTerritories())
		{
			...
			if (Matches.TerritoryIsLand.match(eTerr) && Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data).match(eTerr))
			{
				...
				{
					eTerrValue += AdvUtils.doesLandExistAt(eTerr, data) ? 0.0F : 50.0F;
				}
				*/
			...
			else if (Matches.isTerritoryAllied(player, data).match(eTerr) && Matches.TerritoryIsNotNeutralButCouldBeWater.match(eTerr))
			{
				...
				if (hasAlliedFactory)
				{
					eTerrValue += 4.0F + (hasENeighbors && rankStrength > 5.0F ? rankStrength / 2 : 0.0F);
		...
		// Add in the AI strategy bonus to the territories.
		final Iterator<Territory> lRIter = landRankMap.keySet().iterator();
		while (lRIter.hasNext())
		{
			final Territory checkTerr = lRIter.next();
			if (terrStrat != null && terrStrat.containsKey(checkTerr))
				landRankMap.put(checkTerr, landRankMap.get(checkTerr) + terrStrat.get(checkTerr));
		}
		return landRankMap;
	}
	...
	{
		final List<Territory> blitzers = AdvUtils.possibleBlitzTerritories(eTerr, data, player);
		...
		while (bIter.hasNext())
		{
			...
			if (!blitzCounted && Matches.isTerritoryEnemyAndNotUnownedWaterOrImpassibleOrRestricted(player, data).match(newBTerr))
				nonBlitzStrength += AdvUtils.strength(newBTerr.getUnits().getMatches(Matches.enemyUnit(player, data)), true, false, false);
		}
	...
	public static HashMap<Territory, Float> rankAmphibReinforcementTerritories(final GameData data, final List<Territory> ignoreTerr, final PlayerID player, final boolean tFirst)
	{
		...
		for (final Territory aTerr : data.getMap().getTerritories())
		{
			...
			{
				final Set<Territory> neighbors = data.getMap().getNeighbors(aTerr, Matches.territoryHasAlliedFactory(data, player));
			...
			*/
			if (Matches.territoryHasAlliedNeighborWithAlliedUnitMatching(data, player, Matches.UnitCanProduceUnits).match(capTerr)) // does this need reinforcing?
			{
				...
				{
					final float xValue = AdvUtils.twoAwayStrengthNotCounted(data, player, aTerr);
