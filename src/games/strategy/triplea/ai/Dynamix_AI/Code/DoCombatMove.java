  ...
  public static void doCombatMove(final Dynamix_AI ai, final GameData data, final IMoveDelegate mover, final PlayerID player) {
    ...
    if (true) // Temporary ship movement block
    {
      ...
      for (final Territory ter : data.getMap().getTerritories()) {
        ...
        {
          final Territory closestTerWithOtherShips = DUtils.GetClosestTerMatchingX(data, ter,
              DUtils.CompMatchAnd(Matches.TerritoryIsWater, Matches.territoryIs(ter).invert(),
                  Matches.territoryHasUnitsThatMatch(Matches.unitIsOwnedBy(player))));
          for (final Unit unit : ourUnitGroup) {
        ...
        for (final Territory ter2 : data.getMap().getTerritories()) {
          ...
          final List<Territory> continentTroubleTers =
              DUtils.GetTerritoriesWithinXDistanceOfYMatchingZAndHavingRouteMatchingA(data, ter2, Integer.MAX_VALUE,
                  DUtils.CompMatchAnd(Matches.TerritoryIsLand, Matches.territoryHasEnemyLandUnits(player, data)),
                  DMatches.TerritoryIsLandAndPassable);
  ...
  private static List<CM_Task> GenerateTasks(final MovePackage pack) {
    ...
    final Match<Territory> isAttack_Trade = new Match<Territory>() {
      ...
      public boolean match(final Territory ter) {
        ...
        if (ter.getUnits().getMatches(new CompositeMatchAnd<Unit>(Matches.unitHasDefenseThatIsMoreThanOrEqualTo(1),
            Matches.unitIsEnemyOf(data, player), Matches.UnitIsNotAA)).isEmpty()) {
