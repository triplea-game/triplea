...
public class Purchase {
  public static void purchase(final Dynamix_AI ai, final boolean purchaseForBid, int PUsToSpend,
    ...
    if (!purchaseForBid && DSettings.LoadSettings().EnableResourceCollectionMultiplier
      ...
      if (PUChange > 0) {
        ...
        final String message = ai.getName() + " use an RCM cheat, and increase their PUs from "
            + player.getResources().getQuantity(GlobalCenter.GetPUResource()) + " to " + newPUs;
        DUtils.Log(Level.FINE, message);
        final Runnable runner = new Runnable() {
          @Override
          public void run() {
    ...
    if (purchaseForBid) {
      ...
      if (DUtils.CanPlayerPlaceAnywhere(data, player) && PUsToSpend == 0) {
        DUtils.Log(Level.FINE,
            "  On this map, 'Place-Anywhere' countries can't purchase their units(engine does it for them), so skipping phase.");
        return;
      }
      ...
      DUtils.Log(Level.FINE, "  Factories to build on calculated. Ters with the factories: {0}",
          FactoryCenter.get(data, player).ChosenFactoryTerritories);
      PUsToSpend = PUsToSpend - purchaseFactoryRepairs(ai, purchaseForBid, PUsToSpend, purchaser, data, player);
  ...
  public static int purchaseFactoryRepairs(final Dynamix_AI ai, final boolean purchaseForBid, int PUsToSpend,
      final IPurchaseDelegate purchaser, final GameData data, final PlayerID player) {
  ...
  public static void calculateFactoriesToBuildOn(final Dynamix_AI ai, final boolean purchaseForBid, final GameData data,
      final PlayerID player) {
    final List<Territory> sortedLocations =
        Purchase_UnitPlacementLocationSorter.CalculateAndSortUnitPlacementLocations(ai, purchaseForBid, data, player);
  ...
  private static PurchaseGroup CalculateBestPurchaseGroup(final Territory ter, final GameData data,
                                                                                                                    ...
                                                                                                                    // long
    {
      ...
      if (unitsToBuy.size() >= productionSpaceLeft) {
        break;
      }
