...
public class CM_Task {
  ...
  private List<UnitGroup> getSortedPossibleRecruits() {
    ...
    // Then sort by score. In this way, equal scored attack units are shuffled (Though atm the hashmap sorting method seems to shuffle them
    ...
    if (m_taskType.equals(CM_TaskType.Land_LandGrab) && sortedPossibles.size() > 0) {
      return DUtils.CreateUnitGroupsForUnits(Collections.singletonList(sortedPossibles.get(0)), unitLocations.get(sortedPossibles.get(0)),
          m_data);
    }
  ...
  public void CalculateTaskRequirements() {
    ...
    if (m_taskType == CM_TaskType.Land_Attack_Offensive) {
      if (m_target.getOwner().isNull()) {
        ...
        m_minCounterAttackTradeScoreForBypass =
            DSettings.LoadSettings().TR_attackOffensive_Neutrals_counterAttackTradeScoreRequiredToBypassSurvivalRequirement;
        ...
        m_minCounterAttackTradeScoreForBypass =
            DSettings.LoadSettings().TR_attackOffensive_Capitals_counterAttackTradeScoreRequiredToBypassSurvivalRequirement;
      } else {
      ...
      if (m_target.getOwner().isNull()) {
        ...
        m_attackTrade_certaintyOfReachingLeftoverLUnitsGoalRequired =
            DUtils.ToFloat(DSettings.LoadSettings().TR_attackTrade_certaintyOfReachingDesiredNumberOfLeftoverLandUnitsRequired);
      } else if (DUtils.GetAllEnemyCaps_ThatAreOwnedByOriginalOwner(m_data, GlobalCenter.CurrentPlayer).contains(m_target)) {
        m_attackTrade_tradeScoreRequired = DSettings.LoadSettings().TR_attackTrade_totalTradeScoreRequired;
    ...
    // DUtils.Log(Level.FINER, " CM Task requirements calculated. Min Chance: {0} Min Survival Chance: {1}", m_minTakeoverChance,
    // m_minSurvivalChance);
  }
  ...
  private float getMeetingOfMinTakeoverChance(final AggregateResults simulatedAttack, final float minTakeoverChance) {
    ...
    return DUtils.Divide_SL((float) simulatedAttack.getAttackerWinPercent(), minTakeoverChance); // We're this close to meeting the min
                                                                                                 // takeover chance
  }
  ...
  private float getMeetingOfMinSurvivalChance(final AggregateResults simulatedResponse, final float minSurvivalChance) {
    ...
    return DUtils.Divide_SL((float) simulatedResponse.getDefenderWinPercent(), minSurvivalChance); // We're this close to meeting the min
                                                                                                   // survival chance
  }
  ...
  private void recruitEnoughUnitsToMeetXYZ(final float minTakeoverChance, final float minSurvivalChance, final int maxBattleVolleys) {
    ...
    for (final UnitGroup ug : sortedPossibles) {
      ...
      final AggregateResults simulatedAttack = DUtils.GetBattleResults(GetRecruitedUnitsAsUnitList(),
          DUtils.ToList(m_target.getUnits().getMatches(Matches.unitIsEnemyOf(m_data, GlobalCenter.CurrentPlayer))), m_target, m_data, 1,
          true);
      final List<Unit> responseAttackers = DUtils.DetermineResponseAttackers(m_data, GlobalCenter.CurrentPlayer, m_target, simulatedAttack);
      final List<Unit> responseDefenders = Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air
                                                                                                                                        // can't
                                                                                                                                        // defend
                                                                                                                                        // ter
                                                                                                                                        // because
                                                                                                                                        ...
                                                                                                                                        // to
                                                                                                                                        // land
      final AggregateResults simulatedResponse = DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target, m_data, 1, true);
      final float howCloseToMeetingMinTakeoverChance = getMeetingOfMinTakeoverChance(simulatedAttack, minTakeoverChance);
      if (howCloseToMeetingMinTakeoverChance < 1.0F) {
        m_recruitedUnits.add(ug);
        continue;
      }
      final float howCloseToMeetingMinSurvivalChance = getMeetingOfMinSurvivalChance(simulatedResponse, minSurvivalChance);
    ...
    for (final UnitGroup ug : sortedPossibles) {
      ...
      final List<Unit> responseDefenders = Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air
                                                                                                                                        ...
                                                                                                                                        // to
                                                                                                                                        // land
      final AggregateResults simulatedResponse = DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target, m_data,
          DSettings.LoadSettings().CA_CMNCM_determinesIfTasksRequirementsAreMetEnoughForRecruitingStop, true);
      final float howCloseToMeetingMinTakeoverChance = getMeetingOfMinTakeoverChance(simulatedAttack, minTakeoverChance);
      if (howCloseToMeetingMinTakeoverChance < 1.0F) {
        m_recruitedUnits.add(ug);
  ...
  private void recruitEnoughUnitsForTradeTask(final float certaintyOfReachingLeftoverLUnitsGoalRequired,
    ...
    for (final UnitGroup ug : sortedPossibles) {
      ...
      final List<Unit> defenders = DUtils.ToList(m_target.getUnits().getMatches(Matches.unitIsEnemyOf(m_data, GlobalCenter.CurrentPlayer)));
      final int tradeScore = DUtils.GetTaskTradeScore(m_data, m_target, attackers, defenders, simulatedAttack, responseAttackers,
          responseDefenders, simulatedResponse);
  ...
  public boolean IsPlannedAttackWorthwhile(final List<CM_Task> allTasks) {
    ...
    if (m_target.getOwner().isNull()) {
      ...
      if (DUtils.GetTerritoriesWithinXDistanceOfYMatchingZAndHavingRouteMatchingA(m_data, m_target, Integer.MAX_VALUE,
          DMatches.territoryIsOwnedByNNEnemy(m_data, player),
        ...
        if (GlobalCenter.IsFFAGame && m_target.getUnits().getMatches(Matches.unitHasDefenseThatIsMoreThanOrEqualTo(1)).size() > 0
            && Math.random() < .95F) {
          return false;
        }
    ...
    if (false) // Atm, causes unwanted task cancelling. Was: m_taskType == CM_TaskType.Attack_Offensive)
    {
      for (final UnitGroup ug : new ArrayList<UnitGroup>(m_recruitedUnits)) {
        for (final CM_Task task : allTasks) {
          ...
          {
            if (task.IsDisqualified()) // We're looking for higher priority tasks(higher priority ones should have already been processed)
    ...
    if (m_taskType == CM_TaskType.Land_Attack_Offensive || m_taskType == CM_TaskType.Land_Attack_Trade) {
      ...
      if (closestToTargetEnemyCap != null) {
        ...
        final Route targetToEnemyCapRoute = CachedCalculationCenter.GetPassableLandRoute(data, m_target, closestToTargetEnemyCap);
        for (final Territory recruitFromLoc : DUtils.GetUnitLocations(data, GetRecruitedUnitsAsUnitList())) {
          final Route routeToEnemyCap = CachedCalculationCenter.GetPassableLandRoute(data, recruitFromLoc, closestToTargetEnemyCap);
        ...
        final StrategyType strategyType =
            StrategyCenter.get(m_data, player).GetCalculatedStrategyAssignments().get(closestToTargetEnemyCap.getOwner());
        if (!targetIsTooCloseToCap && strategyType == StrategyType.Enemy_Defensive && targetIsCloserToCapThanFromLocations) {
          ...
          final List<Unit> ourUnitsInArea = DUtils.GetUnitsMatchingXInTerritories(
              DUtils.GetTerritoriesWithinXDistanceOfYMatchingZAndHavingRouteMatchingA(m_data, m_target, 2, Matches.TerritoryIsLand,
    ...
    if (!m_taskType.equals(CM_TaskType.Land_LandGrab)) // If this is a land grab, we don't care if the cap is in danger (We can move the
                                                       // unit back)
    {
    ...
    DUtils.Log(Level.FINEST, "        Attack simulated. Attackers Size: {0} Defenders Size: {1} Takeover Chance: {2}", attackers.size(),
        defenders.size(), simulatedAttack.getAttackerWinPercent());
    DUtils.Log(Level.FINEST, "        Counter attack simulated. Attackers Size: {0} Defenders Size: {1} Takeover Chance: {2}",
        responseAttackers.size(), responseDefenders.size(),
    ...
    if (m_taskType == CM_TaskType.Land_Attack_Offensive) {
      ...
      // For example, you might want the 'meetTUVWants..' method to return even higher than 1.0 if the enemy, for example, loses more than
      // twice as much TUV as us. (In attack and response)
      // That might make an attack go through even if the other things aren't met, which may be good sometimes, like in the example
      ...
      final float howCloseToMeetingTakeoverChanceMin = getMeetingOfMinTakeoverChance(simulatedAttack, m_minTakeoverChance);
      DUtils.Log(Level.FINEST, "        How close to meeting takeover chance min: {0} Needed: {1}", howCloseToMeetingTakeoverChanceMin,
          .98F);
      if (howCloseToMeetingTakeoverChanceMin < .98F) {
        return false;
      }
      final float howCloseToMeetingMinSurvivalChance = getMeetingOfMinSurvivalChance(simulatedResponse, m_minSurvivalChance);
      DUtils.Log(Level.FINEST, "        How close to meeting survival chance min: {0} Needed: {1}", howCloseToMeetingMinSurvivalChance,
          .98F);
      if (howCloseToMeetingMinSurvivalChance < .98F) {
        ...
        simulatedResponse = DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target, m_data,
            DSettings.LoadSettings().CA_CMNCM_determinesResponseResultsToSeeIfTaskWorthwhile,
            false);
        final int tradeScoreIfAttacked = -DUtils.GetTaskTradeScore(m_data, m_target, responseAttackers, responseDefenders,
            simulatedResponse, new ArrayList<Unit>(), new ArrayList<Unit>(),
            null);
      ...
      // Recreate response results with 'toTake' turned off, otherwise enemy counter-attack results show huge loss of TUV for them, if they
      // have no land attacking, which could cause us to attack a ter even if we only have air!
      simulatedResponse = DUtils
          .GetBattleResults(responseAttackers, responseDefenders, m_target, m_data,
      ...
      final Territory startTer = TacticalCenter.get(m_data, player).GetUnitLocationAtStartOfTurn(m_recruitedUnits.get(0).GetFirstUnit()); // Land
                                                                                                                                          // grabs
                                                                                                                                          // are
                                                                                                                                          // done
                                                                                                                                          // with
  ...
  public boolean IsTaskWithAdditionalRecruitsWorthwhile() {
    DUtils.Log(Level.FINEST, "      Determining if cm task with additional recruits is worthwhile. Target: {0} Recruits Size: {1}",
  ...
  /**
   * If this is a cm task that is strong enough to resist all it's threats, we invalidate them because they can't attack more than one
   * place.
   * (This method was added to fix the problem where one far-away airplane 'stack' can discourage ALL our attacks in the area, which is very
   * bad.
  ...
  public void InvalidateThreatsThisTaskResists() {
    final PlayerID player = GlobalCenter.CurrentPlayer;
    if (m_taskType == CM_TaskType.Land_Attack_Offensive) {
      ThreatInvalidationCenter.get(m_data, player).SuspendThreatInvalidation(); // We want to invalidate threats only if this task REALLY
                                                                                // DOES resist ALL ENEMIES, including INVALIDATED ones.
      final AggregateResults simulatedAttack = DUtils.GetBattleResults(GetRecruitedUnitsAsUnitList(),
          DUtils.ToList(m_target.getUnits().getMatches(Matches.unitIsEnemyOf(m_data, GlobalCenter.CurrentPlayer))), m_target, m_data, 250,
          true);
      final List<Unit> threats = DUtils.DetermineResponseAttackers(m_data, GlobalCenter.CurrentPlayer, m_target, simulatedAttack);
      if (threats.isEmpty()) {
        return;
      }
      final List<Unit> responseDefenders = Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air
      ...
      if (simulatedResponse.getDefenderWinPercent() > .4F) {
        ...
        // DUtils.Log(Level.FINER, " Land_Attack_Offensive task succeeded with enough defense, so invalidating threats resisted by this
      ...
      if (simulatedResponse.getDefenderWinPercent() > .4F) {
        ThreatInvalidationCenter.get(m_data, player).InvalidateThreats(threats, m_target);
        // DUtils.Log(Level.FINER, " Land_Attack_Stabalize task succeeded with enough defense, so invalidating threats resisted by this
        // task. Target: {0} Units Invalidated: {1}", m_target, threats);
      }
      ThreatInvalidationCenter.get(m_data, player).ResumeThreatInvalidation();
    }
  ...
  public void Reset() {
    DUtils.Log(Level.FINER, "        Resetting task. Target: {0} Task Type: {1} Priority: {2} Recruit Size: {3}", m_target, m_taskType,
  ...
  public void PerformTask(final IMoveDelegate mover) {
    if (m_recruitedUnits.isEmpty()) {
      DUtils.Log(Level.FINER, "      Task is called to perform, but there are no recruits! Target: {0} Task Type: {1} Priority: {2}",
