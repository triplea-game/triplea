...
public class CM_Task {
  ...
  private List<UnitGroup> getSortedPossibleRecruits() {
    ...
    for (final Territory ter : m_data.getMap().getTerritories()) {
      if (DMatches.territoryContainsMultipleAlliances(m_data).match(ter)) {
        continue;
      }
    ...
    // sortedPossibles = DUtils.ToList(BattleCalculator.sortUnitsForCasualtiesWithSupport(sortedPossibles, false,
    // GlobalCenter.CurrentPlayer, BattleCalculator.getCosts(GlobalCenter.CurrentPlayer, m_data), m_data, false));
    // Then sort by score. In this way, equal scored attack units are shuffled (Though atm the hashmap sorting method
    // seems to shuffle them
    // up even if they're equally scored, so we comment out the other line)
    sortedPossibles = DSorting.SortListByScores_HashMap_D(sortedPossibles, possibles);
    if (m_taskType.equals(CM_TaskType.Land_LandGrab) && sortedPossibles.size() > 0) {
      return DUtils.CreateUnitGroupsForUnits(Collections.singletonList(sortedPossibles.get(0)),
          unitLocations.get(sortedPossibles.get(0)), m_data);
    }
  ...
  public void CalculateTaskRequirements() {
    ...
    if (m_taskType == CM_TaskType.Land_Attack_Offensive) {
      if (m_target.getOwner().isNull()) {
        m_minTakeoverChance =
            DUtils.ToFloat(DSettings.LoadSettings().TR_attackOffensive_Neutrals_takeoverChanceRequired);
        m_minSurvivalChance =
            DUtils.ToFloat(DSettings.LoadSettings().TR_attackOffensive_Neutrals_counterAttackSurvivalChanceRequired);
        m_minCounterAttackTradeScoreForBypass = DSettings
            .LoadSettings().TR_attackOffensive_Neutrals_counterAttackTradeScoreRequiredToBypassSurvivalRequirement;
      } else
        if (DUtils.GetAllEnemyCaps_ThatAreOwnedByOriginalOwner(m_data, GlobalCenter.CurrentPlayer).contains(m_target)) {
        m_minTakeoverChance =
            DUtils.ToFloat(DSettings.LoadSettings().TR_attackOffensive_Capitals_takeoverChanceRequired);
        m_minSurvivalChance =
            DUtils.ToFloat(DSettings.LoadSettings().TR_attackOffensive_Capitals_counterAttackSurvivalChanceRequired);
        m_minCounterAttackTradeScoreForBypass = DSettings
            .LoadSettings().TR_attackOffensive_Capitals_counterAttackTradeScoreRequiredToBypassSurvivalRequirement;
      } else {
        m_minTakeoverChance = DUtils.ToFloat(DSettings.LoadSettings().TR_attackOffensive_takeoverChanceRequired);
        m_minSurvivalChance =
            DUtils.ToFloat(DSettings.LoadSettings().TR_attackOffensive_counterAttackSurvivalChanceRequired);
        m_minCounterAttackTradeScoreForBypass =
            DSettings.LoadSettings().TR_attackOffensive_counterAttackTradeScoreRequiredToBypassSurvivalRequirement;
      }
      ...
      if (m_target.getOwner().isNull()) {
        ...
        m_minSurvivalChance =
            DUtils.ToFloat(DSettings.LoadSettings().TR_attackStabalize_counterAttackSurvivalChanceRequired);
        m_minCounterAttackTradeScoreForBypass =
            DSettings.LoadSettings().TR_attackStabalize_counterAttackTradeScoreRequiredToBypassSurvivalRequirement;
      }
      ...
      if (m_target.getOwner().isNull()) {
        ...
        m_attackTrade_certaintyOfReachingLeftoverLUnitsGoalRequired = DUtils.ToFloat(
            DSettings.LoadSettings().TR_attackTrade_certaintyOfReachingDesiredNumberOfLeftoverLandUnitsRequired);
      } else
        if (DUtils.GetAllEnemyCaps_ThatAreOwnedByOriginalOwner(m_data, GlobalCenter.CurrentPlayer).contains(m_target)) {
        m_attackTrade_tradeScoreRequired = DSettings.LoadSettings().TR_attackTrade_totalTradeScoreRequired;
    ...
    // DUtils.Log(Level.FINER, " CM Task requirements calculated. Min Chance: {0} Min Survival Chance: {1}",
    // m_minTakeoverChance,
    // m_minSurvivalChance);
  }
  ...
  private float getMeetingOfMinTakeoverChance(final AggregateResults simulatedAttack, final float minTakeoverChance) {
    ...
    return DUtils.Divide_SL((float) simulatedAttack.getAttackerWinPercent(), minTakeoverChance); // We're this close to
                                                                                                 // meeting the min
                                                                                                 // takeover chance
  }
  ...
  private float getMeetingOfMinSurvivalChance(final AggregateResults simulatedResponse, final float minSurvivalChance) {
    ...
    return DUtils.Divide_SL((float) simulatedResponse.getDefenderWinPercent(), minSurvivalChance); // We're this close
                                                                                                   // to meeting the min
                                                                                                   // survival chance
  }
  ...
  private void recruitEnoughUnitsToMeetXYZ(final float minTakeoverChance, final float minSurvivalChance,
      final int maxBattleVolleys) {
    ...
    for (final UnitGroup ug : sortedPossibles) {
      ...
      final AggregateResults simulatedAttack = DUtils.GetBattleResults(GetRecruitedUnitsAsUnitList(),
          DUtils.ToList(m_target.getUnits().getMatches(Matches.unitIsEnemyOf(m_data, GlobalCenter.CurrentPlayer))),
          m_target, m_data, 1, true);
      final List<Unit> responseAttackers =
          DUtils.DetermineResponseAttackers(m_data, GlobalCenter.CurrentPlayer, m_target, simulatedAttack);
      final List<Unit> responseDefenders =
          Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air
                                                                                                       // can't
                                                                                                       // defend
                                                                                                       // ter
                                                                                                       // because
                                                                                                       ...
                                                                                                       // to
                                                                                                       // land
      final AggregateResults simulatedResponse =
          DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target, m_data, 1, true);
      final float howCloseToMeetingMinTakeoverChance =
          getMeetingOfMinTakeoverChance(simulatedAttack, minTakeoverChance);
      if (howCloseToMeetingMinTakeoverChance < 1.0F) {
        m_recruitedUnits.add(ug);
        continue;
      }
      final float howCloseToMeetingMinSurvivalChance =
          getMeetingOfMinSurvivalChance(simulatedResponse, minSurvivalChance);
    ...
    for (final UnitGroup ug : sortedPossibles) {
      ...
      final List<Unit> responseDefenders =
          Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air
                                                                                                       ...
                                                                                                       // to
                                                                                                       // land
      final AggregateResults simulatedResponse = DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target,
          m_data, DSettings.LoadSettings().CA_CMNCM_determinesIfTasksRequirementsAreMetEnoughForRecruitingStop, true);
      final float howCloseToMeetingMinTakeoverChance =
          getMeetingOfMinTakeoverChance(simulatedAttack, minTakeoverChance);
      if (howCloseToMeetingMinTakeoverChance < 1.0F) {
        m_recruitedUnits.add(ug);
  ...
  private void recruitEnoughUnitsForTradeTask(final float certaintyOfReachingLeftoverLUnitsGoalRequired,
      final boolean recruitTillTUVStartsDropping) {
    final List<UnitGroup> sortedPossibles = getSortedPossibleRecruits();
    if (sortedPossibles.isEmpty()) {
      return;
    }
    ...
    for (final UnitGroup ug : sortedPossibles) {
      ...
      final AggregateResults simulatedResponse =
          DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target, m_data, 1, true);
      final List<Unit> attackers = GetRecruitedUnitsAsUnitList();
      final List<Unit> defenders =
          DUtils.ToList(m_target.getUnits().getMatches(Matches.unitIsEnemyOf(m_data, GlobalCenter.CurrentPlayer)));
      final int tradeScore = DUtils.GetTaskTradeScore(m_data, m_target, attackers, defenders, simulatedAttack,
          responseAttackers, responseDefenders, simulatedResponse);
      ...
      for (final BattleResults result : simulatedAttack.getResults()) {
        if (Match.getMatches(result.getRemainingAttackingUnits(), Matches.UnitIsLand)
            .size() >= m_attackTrade_leftoverLandUnits) {
          timesWeReachLeftoverLUnitsGoal++;
        }
      }
      final float certaintyOfReachingLUnitsCount =
          (float) timesWeReachLeftoverLUnitsGoal / (float) simulatedAttack.getResults().size();
    ...
    for (final UnitGroup ug : sortedPossibles) {
      ...
      final List<Unit> responseDefenders =
          Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air
                                                                                                       ...
                                                                                                       // land
      final AggregateResults simulatedResponse = DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target,
          m_data, DSettings.LoadSettings().CA_CM_determinesIfTradeTasksRequirementsAreMetEnoughForRecruitingStop, true);
      final List<Unit> attackers = GetRecruitedUnitsAsUnitList();
      final List<Unit> defenders =
          DUtils.ToList(m_target.getUnits().getMatches(Matches.unitIsEnemyOf(m_data, GlobalCenter.CurrentPlayer)));
      final int tradeScore = DUtils.GetTaskTradeScore(m_data, m_target, attackers, defenders, simulatedAttack,
    ...
    m_recruitedUnits = DUtils.TrimRecruits_NonMovedOnes(m_recruitedUnits, successiveNonIncreases); // Go back to when
                                                                                                   // TUV score was
                                                                                                   // highest
  }
  ...
  public boolean IsPlannedAttackWorthwhile(final List<CM_Task> allTasks) {
    ...
    DUtils.Log(Level.FINEST, "      Determining if cm task is worthwhile. Target: {0} Recruits Size: {1}", m_target,
        m_recruitedUnits.size());
    if (m_target.getOwner().isNull()) {
      // If we have at least three non-neutral ters we can walk to and attack, we use these quick-exit checks
      if (DUtils.GetTerritoriesWithinXDistanceOfYMatchingZAndHavingRouteMatchingA(m_data, m_target, Integer.MAX_VALUE,
          DMatches.territoryIsOwnedByNNEnemy(m_data, player), Matches.TerritoryIsLand).size() >= 3) {
        // Atm, AI over-attacks neutrals on FFA maps, so for now, ignore attacks on non-empty neutrals 95% of the time
        // on FFA maps
        if (GlobalCenter.IsFFAGame
            && m_target.getUnits().getMatches(Matches.unitHasDefenseThatIsMoreThanOrEqualTo(1)).size() > 0
            && Math.random() < .95F) {
          return false;
        }
        // Atm, don't attack neutrals if the neutral attack charge is more than the territory production, 75% of the
        // time
        ...
        if (TerritoryAttachment.get(m_target) == null || games.strategy.triplea.Properties
            .getNeutralCharge(m_data) > TerritoryAttachment.getProduction(m_target) * 3) {
          return false;
        }
      }
    }
    ...
    {
      for (final UnitGroup ug : new ArrayList<UnitGroup>(m_recruitedUnits)) {
        for (final CM_Task task : allTasks) {
          if (task.equals(this)) {
            continue;
          }
          if (task.GetPriority() >= this.GetPriority() * 2) // If task is twice as important is this one
          {
            if (task.IsDisqualified()) // We're looking for higher priority tasks(higher priority ones should have
                                       // already been processed)
                                       // that we're currently unable to perform and complete
            {
              if (DMatches.UnitGroupCanReach_Some(task.GetTarget()).match(ug)) {
    ...
    // This code block stops certain attacks on enemies that are calculated to be ones we use the Enemy_Defensive
    // strategy on
    if (m_taskType == CM_TaskType.Land_Attack_Offensive || m_taskType == CM_TaskType.Land_Attack_Trade) {
      final GameData data = m_target.getData();
      final Territory closestToTargetEnemyCap =
          DUtils.GetClosestTerInList(data, DUtils.GetAllEnemyCaps_ThatAreOwnedByOriginalOwner(data, player), m_target);
      if (closestToTargetEnemyCap != null) {
        boolean targetIsCloserToCapThanFromLocations = true;
        final Route targetToEnemyCapRoute =
            CachedCalculationCenter.GetPassableLandRoute(data, m_target, closestToTargetEnemyCap);
        for (final Territory recruitFromLoc : DUtils.GetUnitLocations(data, GetRecruitedUnitsAsUnitList())) {
          final Route routeToEnemyCap =
              CachedCalculationCenter.GetPassableLandRoute(data, recruitFromLoc, closestToTargetEnemyCap);
          if (routeToEnemyCap != null && targetToEnemyCapRoute != null
              && routeToEnemyCap.getLength() <= targetToEnemyCapRoute.getLength()) {
            targetIsCloserToCapThanFromLocations = false;
            break;
          }
        }
        boolean targetIsTooCloseToCap = false;
        final Territory ourClosestCap = DUtils.GetClosestTerInList(data, DUtils.GetAllOurCaps(data, player), m_target);
        final Route routeToClosestCap = CachedCalculationCenter.GetPassableLandRoute(data, m_target, ourClosestCap);
        if (ourClosestCap != null && routeToClosestCap != null
            && routeToClosestCap.getLength() <= (3.0F * GlobalCenter.MapTerCountScale)) {
          targetIsTooCloseToCap = true;
        }
        final StrategyType strategyType = StrategyCenter.get(m_data, player).GetCalculatedStrategyAssignments()
            .get(closestToTargetEnemyCap.getOwner());
        if (!targetIsTooCloseToCap && strategyType == StrategyType.Enemy_Defensive
            && targetIsCloserToCapThanFromLocations) {
          // For now, if this attack group makes up over half of the units we have in the area, cancel the move
          final List<Unit> ourUnitsInArea = DUtils.GetUnitsMatchingXInTerritories(
              DUtils.GetTerritoriesWithinXDistanceOfYMatchingZAndHavingRouteMatchingA(m_data, m_target, 2,
    ...
    if (!m_taskType.equals(CM_TaskType.Land_LandGrab)) // If this is a land grab, we don't care if the cap is in danger
                                                       // (We can move the
                                                       // unit back)
    {
    ...
    final AggregateResults simulatedAttack = DUtils.GetBattleResults(attackers, defenders, m_target, m_data,
        DSettings.LoadSettings().CA_CM_determinesAttackResultsToSeeIfTaskWorthwhile, true);
    final List<Unit> responseAttackers =
        DUtils.DetermineResponseAttackers(m_data, GlobalCenter.CurrentPlayer, m_target, simulatedAttack);
    final List<Unit> responseDefenders =
        Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air
    ...
    DUtils.Log(Level.FINEST, "        Attack simulated. Attackers Size: {0} Defenders Size: {1} Takeover Chance: {2}",
        attackers.size(), defenders.size(), simulatedAttack.getAttackerWinPercent());
    DUtils.Log(Level.FINEST,
        "        Counter attack simulated. Attackers Size: {0} Defenders Size: {1} Takeover Chance: {2}",
        responseAttackers.size(), responseDefenders.size(), simulatedResponse.getAttackerWinPercent());
    if (m_taskType == CM_TaskType.Land_Attack_Offensive) {
      ...
      // If you think it'll help, you can change these 'getMeetingOf...' methods so they return a value higher than 1.0F
      // or lower than 0.0F.
      // For example, you might want the 'meetTUVWants..' method to return even higher than 1.0 if the enemy, for
      // example, loses more than
      // twice as much TUV as us. (In attack and response)
      // That might make an attack go through even if the other things aren't met, which may be good sometimes, like in
      // the example
      // situation where we'd make the enemy lose a lot more TUV.
      // If you do make these sort of changes, though, please do it carefully, sloppy changes could complicate the code.
      final float howCloseToMeetingTakeoverChanceMin =
          getMeetingOfMinTakeoverChance(simulatedAttack, m_minTakeoverChance);
      DUtils.Log(Level.FINEST, "        How close to meeting takeover chance min: {0} Needed: {1}",
          howCloseToMeetingTakeoverChanceMin, .98F);
      if (howCloseToMeetingTakeoverChanceMin < .98F) {
        return false;
      }
      final float howCloseToMeetingMinSurvivalChance =
          getMeetingOfMinSurvivalChance(simulatedResponse, m_minSurvivalChance);
      DUtils.Log(Level.FINEST, "        How close to meeting survival chance min: {0} Needed: {1}",
      ...
      if (howCloseToMeetingMinSurvivalChance < .98F) {
        ...
        simulatedResponse = DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target, m_data,
            DSettings.LoadSettings().CA_CMNCM_determinesResponseResultsToSeeIfTaskWorthwhile, false);
        final int tradeScoreIfAttacked = -DUtils.GetTaskTradeScore(m_data, m_target, responseAttackers,
            responseDefenders, simulatedResponse, new ArrayList<Unit>(), new ArrayList<Unit>(), null);
      ...
      // Recreate response results with 'toTake' turned off, otherwise enemy counter-attack results show huge loss of
      // TUV for them, if they
      // have no land attacking, which could cause us to attack a ter even if we only have air!
      simulatedResponse = DUtils.GetBattleResults(responseAttackers, responseDefenders, m_target, m_data,
          DSettings.LoadSettings().CA_CMNCM_determinesResponseResultsToSeeIfTaskWorthwhile, false);
      final int tradeScore = DUtils.GetTaskTradeScore(m_data, m_target, attackers, defenders, simulatedAttack,
          responseAttackers, responseDefenders, simulatedResponse);
      DUtils.Log(Level.FINEST, "        Task trade score: {0} Needed: {1}", tradeScore,
          m_attackTrade_tradeScoreRequired);
      if (tradeScore < m_attackTrade_tradeScoreRequired) {
        return false;
      }
      int timesWeReachLeftoverLUnitsGoal = 0;
      ...
      final float certaintyOfReachingLUnitsCount =
          (float) timesWeReachLeftoverLUnitsGoal / (float) simulatedAttack.getResults().size();
      DUtils.Log(Level.FINEST, "        Certainty of reaching leftover land units goal({0}): {1} Needed: {2}",
          m_attackTrade_leftoverLandUnits, certaintyOfReachingLUnitsCount,
      ...
      if (CachedInstanceCenter.CachedBattleTracker.wasConquered(m_target)) // If the blitz target was already taken
      {
        ...
        DUtils.Log(Level.FINEST,
            "        Territory to-grab was already grabbed, so not doing moving, but marking task as completed.");
      ...
      final Territory startTer =
          TacticalCenter.get(m_data, player).GetUnitLocationAtStartOfTurn(m_recruitedUnits.get(0).GetFirstUnit()); // Land
                                                                                                                   // grabs
                                                                                                                   // are
                                                                                                                   // done
                                                                                                                   // with
                                                                                                                   ...
                                                                                                                   // one
                                                                                                                   // unit
      final Route route = m_data.getMap().getRoute(startTer, m_target, Matches.TerritoryIsLand);
      boolean canUnitsGetBack = false;
      for (final Unit unit : GetRecruitedUnitsAsUnitList()) {
      ...
      if (landAttackers.size() > 0) {
        cheapestLAttacker =
            DUtils.GetTUVOfUnit(DUtils.GetCheapestUnitInList(landAttackers), GlobalCenter.GetPUResource());
      }
      final float chanceOfHittingCheapestLAttacker =
          UnitAttachment.get(GetRecruitedUnitsAsUnitList().get(0).getUnitType()).getDefense(player) / 6.0F;
      final float averageTuvLossOfAttackerIfAttacked = chanceOfHittingCheapestLAttacker * cheapestLAttacker;
  ...
  public boolean IsTaskWithAdditionalRecruitsWorthwhile() {
    DUtils.Log(Level.FINEST,
        "      Determining if cm task with additional recruits is worthwhile. Target: {0} Recruits Size: {1}", m_target,
        m_recruitedUnits.size());
    if (m_recruitedUnits.isEmpty()) {
      return false;
    }
      ...
      if (areRecruitsFromCapsOrNeighbors && !ourCaps.contains(m_target)) {
        ...
        if (capTakeoverChances.get(1) > .1F) // If takeover chance is 10% or more after move
        {
          ...
          if (capTakeoverChances.get(1) - capTakeoverChances.get(0) > .01F
              || capTakeoverChances.get(3) - capTakeoverChances.get(2) > 1) {
            DUtils.Log(Level.FINEST,
                "      Performing task with additional recruits would endanger capital, so canceling.");
            return false;
          }
        }
      }
  ...
  /**
   * If this is a cm task that is strong enough to resist all it's threats, we invalidate them because they can't attack
   * more than one
   * place.
   * (This method was added to fix the problem where one far-away airplane 'stack' can discourage ALL our attacks in the
   * area, which is very
   * bad.
   * Now, if one task can resist the plane stack, we assume the other movements are partially 'safe' from this enemy
   * stack)
   */
  public void InvalidateThreatsThisTaskResists() {
    final PlayerID player = GlobalCenter.CurrentPlayer;
    if (m_taskType == CM_TaskType.Land_Attack_Offensive) {
      ThreatInvalidationCenter.get(m_data, player).SuspendThreatInvalidation(); // We want to invalidate threats only if
                                                                                // this task REALLY
                                                                                // DOES resist ALL ENEMIES, including
                                                                                // INVALIDATED ones.
      final AggregateResults simulatedAttack = DUtils.GetBattleResults(GetRecruitedUnitsAsUnitList(),
          DUtils.ToList(m_target.getUnits().getMatches(Matches.unitIsEnemyOf(m_data, GlobalCenter.CurrentPlayer))),
          m_target, m_data, 250, true);
      final List<Unit> threats =
          DUtils.DetermineResponseAttackers(m_data, GlobalCenter.CurrentPlayer, m_target, simulatedAttack);
      if (threats.isEmpty()) {
        return;
      }
      final List<Unit> responseDefenders =
          Match.getMatches(simulatedAttack.GetAverageAttackingUnitsRemaining(), Matches.UnitIsNotAir); // Air
                                                                                                       ...
                                                                                                       // to
                                                                                                       // land
      final AggregateResults simulatedResponse = DUtils.GetBattleResults(threats, responseDefenders, m_target, m_data,
      ...
      if (simulatedResponse.getDefenderWinPercent() > .4F) {
        ...
        // DUtils.Log(Level.FINER, " Land_Attack_Offensive task succeeded with enough defense, so invalidating threats
        // resisted by this
        // task. Target: {0} Units Invalidated: {1}", m_target, threats);
      }
      ThreatInvalidationCenter.get(m_data, player).ResumeThreatInvalidation();
    } else if (m_taskType == CM_TaskType.Land_Attack_Stabilize) {
      ThreatInvalidationCenter.get(m_data, player).SuspendThreatInvalidation(); // We want to invalidate threats only if
      ...
      if (simulatedResponse.getDefenderWinPercent() > .4F) {
        ThreatInvalidationCenter.get(m_data, player).InvalidateThreats(threats, m_target);
        // DUtils.Log(Level.FINER, " Land_Attack_Stabalize task succeeded with enough defense, so invalidating threats
        // resisted by this
        // task. Target: {0} Units Invalidated: {1}", m_target, threats);
      }
      ThreatInvalidationCenter.get(m_data, player).ResumeThreatInvalidation();
  ...
  public void Reset() {
    DUtils.Log(Level.FINER, "        Resetting task. Target: {0} Task Type: {1} Priority: {2} Recruit Size: {3}",
        m_target, m_taskType, m_priority, m_recruitedUnits.size());
    m_completed = false;
  ...
  public void PerformTask(final IMoveDelegate mover) {
    if (m_recruitedUnits.isEmpty()) {
      DUtils.Log(Level.FINER,
          "      Task is called to perform, but there are no recruits! Target: {0} Task Type: {1} Priority: {2}",
          m_target, m_taskType, m_priority);
      m_completed = true;
    ...
    for (final UnitGroup ug : m_recruitedUnits) {
      ...
      final String error = ug.MoveAsFarTo_CM(m_target, mover);
      if (error != null) {
    ...
    ReconsiderSignalCenter.get(m_data, GlobalCenter.CurrentPlayer).ObjectsToReconsider
        .addAll(CachedInstanceCenter.CachedGameData.getMap().getNeighbors(m_target));
    m_completed = true;
    if (m_taskType.equals(CM_TaskType.Land_Attack_Offensive) || m_taskType.equals(CM_TaskType.Land_Attack_Stabilize)) {
      TacticalCenter.get(m_data, GlobalCenter.CurrentPlayer).BattleRetreatChanceAssignments.put(m_target,
          m_minTakeoverChance);
    }
