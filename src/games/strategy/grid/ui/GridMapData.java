...
public class GridMapData {
  ...
  public Territory getTerritoryAt(final double x, final double y, final GameMap map) {
    ...
    if (normal_x < 0 || normal_y < 0 || normal_x > ((m_gridWidth * m_squareWidth) + (m_bevelWidth))
        || normal_y > ((m_gridHeight * m_squareHeight) + (m_bevelHeight))) {
      return null;
    }
  ...
  public Dimension getMapDimensions() {
    return new Dimension(((m_gridWidth * m_squareWidth) + (m_bevelWidth * 2) + 1),
        ((m_gridHeight * m_squareHeight) + (m_bevelHeight * 2) + 1));
  }
}
