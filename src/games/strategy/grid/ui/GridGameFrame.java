...
public class GridGameFrame extends MainGameFrame {
  ...
  public GridGameFrame(final IGame game, final LocalPlayers players,
    ...
    // m_mapData = new GridMapData(m_data, x_dim, y_dim, SQUARE_SIZE, SQUARE_SIZE, OUTSIDE_BEVEL_SIZE,
    // OUTSIDE_BEVEL_SIZE);
    try {
    ...
    try {
      final Constructor<? extends GridGameMenu<GridGameFrame>> menuConstructor =
          menuBarClass.getConstructor(new Class[] {GridGameFrame.class});
  ...
  /**
   * This only applies to the UI for this local machine. Therefore it returns the "last" active player that was played
   * on this machine.
   */
