...
public class GoMapPanel extends GridMapPanel {
  ...
  protected boolean m_wantToContinuePlaying = false; // after we get to end game, if players disagree on score, game
                                                     // will continue
  ...
  public GoMapPanel(final GameData data, final GridMapData mapData, final GridGameFrame parentGridGameFrame,
      final ImageScrollModel imageScrollModel) {
  ...
  protected Triple<List<Territory>, List<Tuple<Territory, Collection<Territory>>>, List<Territory>> getAllPossibleMovesList(
  ...
  protected void paintComponentMiddleLayer(final Graphics2D g2d, final int topLeftX, final int topLeftY) {
    ...
    // we want to draw the squares offset, so that it looks like we are placing pieces on the intersections of lines,
    // not in a square
  ...
  protected void paintValidMoves(final Graphics2D g2d, final int topLeftX, final int topLeftY) {
    if (m_parentGridGameFrame.isGameOver() || m_phase == GO_DELEGATE_PHASE.ENDGAME) {
      ...
      final Map<Territory, PlayerID> currentScoreState =
          EndTurnDelegate.getCurrentAreaScoreState(m_territoryGroupsThatShouldDie, m_gameData);
  ...
  @Override
  public GridPlayData waitForPlay(final PlayerID player, final IPlayerBridge bridge, final CountDownLatch waiting)
  ...
  // TODO: move all end game input stuff to play delegate (or a new delegate), and leave the end turn delegate to just
  // pbem input
  @Override
  public IGridEndTurnData waitForEndTurn(final PlayerID player, final IPlayerBridge bridge,
    ...
    // if we are reloading a pbem game, we don't want to re-do the last player's turn (this would be fixed if we moved
    // this stuff to play
    // delegate)
  ...
  // TODO: such a hack. do it right by moving the end game crap to play delegate, and have the only interaction in end
  // turn be the forum
  // poster
