...
public class EndTurnDelegate extends AbstractPlayByEmailOrForumDelegate implements IGoEndTurnDelegate {
  ...
  public static Tuple<Tuple<PlayerID, Integer>, Tuple<PlayerID, Integer>> getFinalScores(
    ...
    return new Tuple<Tuple<PlayerID, Integer>, Tuple<PlayerID, Integer>>(new Tuple<PlayerID, Integer>(p1, score1),
        new Tuple<PlayerID, Integer>(p2, score2));
  }
  ...
  // (the only interaction during EndTurnDelegate should be the forum poster. this is because otherwise the forum poster will not post the
  // full game data with the interactions included.)
  @Override
    ...
    if (!groupsThatShouldDie.getWantToContinuePlaying()) {
      ...
      if (oldGroupsFromLastPlayer == null) {
        ...
        if (m_player.equals(oldGroupsFromLastPlayer.getFirst())) {
          ...
          text = null;
          return null;
        } else if (!oldGroupsFromLastPlayer.getSecond().getTerritoryUnitsRemovalAdjustment()
            .equals(groupsThatShouldDie.getTerritoryUnitsRemovalAdjustment())) {
