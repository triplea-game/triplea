...
public class HeuristicAI extends GridAbstractAI {
  ...
  static int getPointsForBoardSituation(final PlayerID me, final PlayerID enemy, final Collection<Territory> captures,
    ...
    // final boolean canRecaptureUs = !PlayDelegate.getMovesThatCaptureThisTerritory(temp.getSecond(), enemyTemp, temp.getForth(), 1,
    // true).isEmpty();
    // points += (canRecaptureUs ? 0 : 1);
    final Collection<Tuple<Territory, List<Tuple<Territory, Territory>>>> capturedPieces =
        PlayDelegate.whichOfOurPiecesCanBeCaptured(me, data);
  ...
  static List<Triple<Territory, Territory, Collection<Territory>>> getAllAvailableMoves(final PlayerID player,
    ...
    // triple<start, end, captures>
    final List<Triple<Territory, Territory, Collection<Territory>>> availableMoves =
        new ArrayList<Triple<Territory, Territory, Collection<Territory>>>();
  ...
  static Comparator<Triple<Territory, Territory, Collection<Territory>>> getBestCaptureComparator(final PlayerID player,
      final GameData data) {
    return new Comparator<Triple<Territory, Territory, Collection<Territory>>>() {
      @Override
      public int compare(final Triple<Territory, Territory, Collection<Territory>> t1,
          final Triple<Territory, Territory, Collection<Territory>> t2) {
        if ((t1 == null && t2 == null) || t1 == t2) {
          return 0;
        }
        if (t1 == null && t2 != null) {
