...
public class MapPanel extends JComponent implements MouseListener
{
	...
	public MapPanel(final MapData mapData)
	{
		...
		m_imageFactory.setResourceLoader(ResourceLoader.getMapResourceLoader(mapName));
	 ...
	 */
	protected void performPlay(final Territory start, final Territory end, final Unit unit, final Map<Territory, Unit> captured)
	{
		updateImage(start);
		updateImage(end);
		for (final Territory at : captured.keySet())
			updateImage(at);
	...
	public PlayData waitForPlay(final PlayerID player, final IPlayerBridge bridge, final CountDownLatch waiting) throws InterruptedException
	{
		...
		else if (m_clickedAt.getUnits().getUnits().size() <= 0)
		{
			// no units where we clicked
			// so reset
			m_clickedAt = null;
			m_releasedAt = null;
			return null;
		...
		else
		{
			...
			final PlayData play = new PlayData(m_clickedAt, m_releasedAt, m_clickedAt.getUnits().getUnits().iterator().next());
			m_clickedAt = null;
			m_releasedAt = null;
			return play;
		}
