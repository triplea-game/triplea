    ...
    while (matcherTag.find()) {
      ...
      while (matcherLink.find()) {
        ...
        if (fullLink != null && fullLink.length() > 2) {
          ...
          // System.out.println("Link: " + link);
          final String imageFileName = link.substring(Math.max((link.lastIndexOf("/") + 1), 0)); // remove full parent
                                                                                                 // path
          URL replacementURL = ourResourceLoader.getResource(ASSET_IMAGE_FOLDER + imageFileName); // replace when
                                                                                                  // testing with:
                                                                                                  // "REPLACEMENTPATH/"
                                                                                                  // + imageFileName;
