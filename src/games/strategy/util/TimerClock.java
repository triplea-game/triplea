...
public class TimerClock<T> extends Observable {
  ...
  public T start(final Runnable task, final T defaultReturnValue, final int interruptAfterSecondsIfNotFinished,
    ...
    final AtomicReference<RuntimeException> exception = new AtomicReference<RuntimeException>(); // we want to catch
                                                                                                 // exceptions and
                                                                                                 // propagate them back
                                                                                                 // up
    // start the task
    ...
    if (!runnableFinishedSuccessfully.get() && !runnableHadRuntimeException.get()) {
      ...
      try {
        Thread.sleep(1000); // wait a second to gracefully allow a remote player to receive the notice that they are out
                            // of time
