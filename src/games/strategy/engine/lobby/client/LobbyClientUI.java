 ...
 * @author Harry
 */
public class LobbyClientUI extends JFrame implements WindowListener
{
    ChatPanel m_cp;
    ServersPanel m_sp;
    ...
    public LobbyClientUI(String name,String server,int port,MainFrame frame)
    {
        m_frame = frame;
        ...
        try
        {
            SwingUtilities.invokeLater(
                new Runnable(){
                    public void run()
                    {
                        new LobbyClientUI(args[0],args[1],Integer.parseInt(args[2]),null);
                    }
                }
            );
        }
