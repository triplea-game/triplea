...
public class GameSettingsPanel extends DynamicRowsPanel {
	...
	private TreeSet<String> m_settingNames = new TreeSet<String>();
	...
	{
		if (s_me == null || !(s_me instanceof GameSettingsPanel))
			s_me = new GameSettingsPanel(stepActionPanel);
	...
	protected void layoutComponents()
	{
		...
		setRows(gbl_stepActionPanel, MapXMLHelper.s_gameSettings.size());
		...
		m_ownPanel.add(lSettingName, gbc_lSettingName);
		...
		m_ownPanel.add(lEditable, gbc_lEditable);
		
		GridBagConstraints gbc_lMinNumber = (GridBagConstraints) gbc_lSettingName.clone();
		gbc_lMinNumber.gridx = 3;
		m_ownPanel.add(lMinNumber, gbc_lMinNumber);
		
		GridBagConstraints gbc_lMaxNumber = (GridBagConstraints) gbc_lSettingName.clone();
		gbc_lMaxNumber.gridx = 4;
		m_ownPanel.add(lMaxNumber, gbc_lMaxNumber);
		
		// <3> Add Main Input Rows
		int yValue = 1;

		final String[] settingNames = m_settingNames.toArray(new String[m_settingNames.size()]);
		for (final Entry<String, ArrayList<String>> settingEntry : MapXMLHelper.s_gameSettings.entrySet())
		{
			GridBagConstraints gbc_tValue = (GridBagConstraints) gbc_lSettingName.clone();
			gbc_tValue.gridx = 0;
			gbc_lValue.gridy = yValue;
			final ArrayList<String> settingValue = settingEntry.getValue();
			...
			final GameSettingsRow newRow = new GameSettingsRow(this, m_ownPanel, settingEntry.getKey(), settingNames, settingValue.get(0), settingValue.get(1), minValueInteger, maxValueInteger);
			newRow.addToComponent(m_ownPanel, yValue, gbc_tValue);
			m_rows.add(newRow);
			++yValue;
		}
		...
		bAddValue.addActionListener(new AbstractAction("Add Game Setting")
		{
			...
			{
				final String suggestedSettingName = (String) JOptionPane.showInputDialog(m_ownPanel, "Which game setting should be added?", "Choose Game Setting", JOptionPane.QUESTION_MESSAGE, null,
							m_settingNames.toArray(new String[m_settingNames.size()]), // Array of choices
							m_settingNames.iterator().next()); // Initial choice
				...
				newSettingValue.add("0");
				MapXMLHelper.s_gameSettings.put(suggestedSettingName, newSettingValue);
				
				// UI Update
				setRows((GridBagLayout) m_ownPanel.getLayout(), MapXMLHelper.s_gameSettings.size());
	...
	private DynamicRow addRowWith(final String settingName, final String newValue, final String editable, final int minNumber, final int maxNumber)
	{
		final GameSettingsRow newRow = new GameSettingsRow(this, m_ownPanel, settingName, m_settingNames.toArray(new String[m_settingNames.size()]), newValue, editable, minNumber, maxNumber);
		addRow(newRow);
		return newRow;
	}
	...
	protected void initializeSpecifics()
	{
		...
		m_settingNames.addAll(Arrays.asList(s_allSettings.split(", ")));
		for (final String player : MapXMLHelper.s_playerName)
		{
			m_settingNames.add(player + " bid");
		}
		for (final String ally : MapXMLHelper.s_playerAlliance.values())
		{
			m_settingNames.add(ally + " Honorable Victory VCs");
		}
	}
