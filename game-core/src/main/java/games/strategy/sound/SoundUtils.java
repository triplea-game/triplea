...
public final class SoundUtils {
   ...
   */
  public static void playBattleType(final PlayerId attacker, final List<Unit> attackingUnits,
      final List<Unit> defendingUnits, final IDelegateBridge bridge) {
  ...
  public static void playRetreatType(final PlayerId attacker, final Collection<Unit> units,
      final RetreatType retreatType, final IDelegateBridge bridge) {
    switch (retreatType) {
      case SUBS:
  ...
  /**
   * Plays appropriate sound clip for successful air or sea battle. No sound clips for a successful land battle because
   * a territory will be captured which has its own sound clips.
   */
  public static void playAttackerWinsAirOrSea(final PlayerId attacker, final List<Unit> attackingUnits,
      final boolean isWater, final IDelegateBridge bridge) {
    if (isWater) {
      if (!attackingUnits.isEmpty() && attackingUnits.stream().allMatch(Matches.unitIsAir())) {
        ...
        bridge.getSoundChannelBroadcaster().playSoundForAll(SoundPath.CLIP_BATTLE_SEA_SUCCESSFUL, attacker);
      }
    } else {
      if (!attackingUnits.isEmpty() && attackingUnits.stream().allMatch(Matches.unitIsAir())) {
        bridge.getSoundChannelBroadcaster().playSoundForAll(SoundPath.CLIP_BATTLE_AIR_SUCCESSFUL, attacker);
