...
public class ProOddsCalculator {
  ...
  private static ProBattleResult checkIfNoAttackersOrDefenders(
    ...
    } else if (Properties.getSubRetreatBeforeBattle(data)
  ...
  public ProBattleResult callBattleCalculator(
    ...
    final List<Unit> mainCombatAttackers =
        CollectionUtils.getMatches(
            attackingUnits, Matches.unitCanBeInBattle(true, !t.isWater(), 1, false, true, true));
    final List<Unit> mainCombatDefenders =
        CollectionUtils.getMatches(
            defendingUnits, Matches.unitCanBeInBattle(false, !t.isWater(), 1, false, true, true));
    double tuvSwing =
