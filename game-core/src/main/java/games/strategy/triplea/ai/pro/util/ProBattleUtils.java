...
public class ProBattleUtils {
  ...
  public static boolean checkForOverwhelmingWin(
    ...
    final int totalDefenderHitPoints =
        BattleCalculator.getTotalHitpointsLeft(defendersWithHitPoints);
   ...
   * units.
   *
   * @return A value in the range [0,100]. 0 indicates absolute defender strength; 100 indicates
   *     absolute attacker strength; 50 indicates equal attacker and defender strength.
   */
  ...
  public static double estimateStrength(
    ...
    List<Unit> unitsThatCanFight =
        CollectionUtils.getMatches(
            myUnits, Matches.unitCanBeInBattle(attacking, !t.isWater(), 1, false, true, true));
  ...
  public static boolean territoryHasLocalLandSuperiority(
    ...
    for (int i = 2; i <= distance; i++) {
      ...
      for (final Territory nearbyTerritory : nearbyTerritoriesForAllied) {
        alliedUnits.addAll(
            nearbyTerritory.getUnits().getMatches(Matches.isUnitAllied(player, data)));
      }
  ...
  public static boolean territoryHasLocalNavalSuperiority(
    ...
    for (final Territory nearbySeaTerritory : nearbyAlliedSeaTerritories) {
      ...
      alliedUnitsInSeaTerritories.addAll(
          nearbySeaTerritory.getUnits().getMatches(ProMatches.unitIsAlliedNotOwned(player, data)));
    }
    ProLogger.trace(
        t
            + ", enemyDistance="
