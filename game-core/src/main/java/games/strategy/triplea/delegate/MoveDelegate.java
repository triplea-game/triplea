...
public class MoveDelegate extends AbstractMoveDelegate {
        ...
        if (Matches.unitIsSea().test(u)) {
          ...
          for (final Territory current : neighbors) {
            givesBonusUnits.addAll(
                CollectionUtils.getMatches(current.getUnits().getUnits(), givesBonusUnitLand));
          }
          ...
          for (final Territory current : neighbors) {
            givesBonusUnits.addAll(
                CollectionUtils.getMatches(current.getUnits().getUnits(), givesBonusUnitSea));
          }
        }
  ...
  static void repairMultipleHitPointUnits(final IDelegateBridge bridge, final PlayerId player) {
    ...
    for (final Territory current : data.getMap().getTerritories()) {
      ...
      if (!Properties.getTwoHitPointUnitsRequireRepairFacilities(data)) {
        damaged =
            new HashSet<>(
                current.getUnits().getMatches(repairOnlyOwn ? damagedUnitsOwned : damagedUnits));
      } else {
        damaged =
            new HashSet<>(
                current
    ...
    for (final Unit carrier : damagedCarriers) {
      final CompositeChange change =
          MustFightBattle.clearTransportedByForAlliedAirOnCarrier(
  ...
  private static int getLargestRepairRateForThisUnit(
    ...
    final Set<Unit> repairUnitsForThisUnit =
        new HashSet<>(territoryUnitIsIn.getUnits().getMatches(repairUnit));
    ...
    for (final PlayerId player : data.getPlayerList()) {
      ...
      if (!player.equals(this.player)) {
        util.removeAirThatCantLand(
            player,
            ((player.getUnits().anyMatch(Matches.unitIsCarrier()) || hasProducedCarriers)
                && lhtrCarrierProd));
      }
    }
  }
