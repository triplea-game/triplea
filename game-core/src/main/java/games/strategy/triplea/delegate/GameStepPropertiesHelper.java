...
public final class GameStepPropertiesHelper {
  ...
  private static boolean isNonCombatDelegate(final GameDataInjections data) {
    return data.getSequence().getStep().getName().endsWith("NonCombatMove");
  }

  private static boolean isCombatDelegate(final GameDataInjections data) {
    // NonCombatMove endsWith CombatMove so check for NCM first
    return !data.getSequence().getStep().getName().endsWith("NonCombatMove")
        && data.getSequence().getStep().getName().endsWith("CombatMove");
  }

  private static boolean isAirborneDelegate(final GameDataInjections data) {
    return data.getSequence().getStep().getName().endsWith("AirborneCombatMove");
  }

  private static boolean isBidPurchaseDelegate(final GameDataInjections data) {
    return data.getSequence().getStep().getName().endsWith("Bid");
  }

  private static boolean isBidPlaceDelegate(final GameDataInjections data) {
    return data.getSequence().getStep().getName().endsWith("BidPlace");
  }

  private static boolean isPurchaseDelegate(final GameDataInjections data) {
