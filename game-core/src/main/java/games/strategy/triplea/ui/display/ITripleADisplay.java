...
public interface ITripleADisplay extends IDisplay {
   ...
   */
  void showBattle(GUID battleId, Territory location, Collection<Unit> attackingUnits,
   ...
   */
  void listBattleSteps(List<String> steps);

  /**
   * The given battle has ended.
   */
  void battleEnd(String message);

  /**
   * Notify that the casualties occurred.
   */
  void casualtyNotification(String step, DiceRoll dice, PlayerID player, Collection<Unit> killed,
      Collection<Unit> damaged, Map<Unit, Collection<Unit>> dependents);

  /**
   * Notify that the casualties occurred, and only the casualty.
   */
  void deadUnitNotification(PlayerID player, Collection<Unit> dead,
      Map<Unit, Collection<Unit>> dependents);

  void changedUnitsNotification(PlayerID player, Collection<Unit> removedUnits,
      Collection<Unit> addedUnits, Map<Unit, Collection<Unit>> dependents);

  /**
   * Notification of the results of a bombing raid.
   */
  void bombingResults(List<Die> dice, int cost);

  /**
   * Notify that the given player has retreated some or all of his units.
   */
  void notifyRetreat(String message, String step, PlayerID retreatingPlayer);

  void notifyRetreat(Collection<Unit> retreating);
  ...
  void gotoBattleStep(String step);
}
