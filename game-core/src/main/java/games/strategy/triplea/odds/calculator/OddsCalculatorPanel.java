...
class OddsCalculatorPanel extends JPanel {
    ...
    new Thread(
            () -> {
              try {
                ...
                if (landBattleCheckBox.isSelected()
    ...
    if (results.get() == null) {
      ...
      final List<Unit> mainCombatAttackers =
          CollectionUtils.getMatches(
              attackers.get(), Matches.unitCanBeInBattle(true, isLand, 1, false, true, true));
      final List<Unit> mainCombatDefenders =
          CollectionUtils.getMatches(
              defenders.get(), Matches.unitCanBeInBattle(false, isLand, 1, false, true, true));
  ...
  private void updateDefender(final List<Unit> initialUnits) {
    ...
    defendingUnitsPanel.init(
        getDefender(),
        CollectionUtils.getMatches(
            units, Matches.unitCanBeInBattle(false, isLand, 1, false, false, false)),
        isLand);
  }
  ...
  private void updateAttacker(final List<Unit> initialUnits) {
    ...
    attackingUnitsPanel.init(
        getAttacker(),
        CollectionUtils.getMatches(
            units, Matches.unitCanBeInBattle(true, isLand, 1, false, false, false)),
        isLand);
  }
      ...
      final int attackPower =
          DiceRoll.getTotalPower(
              DiceRoll.getUnitPowerAndRollsForNormalBattles(
                  ...
                  territoryEffects,
                  isAmphibiousBattle,
                  (isAmphibiousBattle ? attackers : new ArrayList<>())),
              data);
