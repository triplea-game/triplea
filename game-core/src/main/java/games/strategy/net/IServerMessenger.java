...
public interface IServerMessenger extends IMessenger {
  ...
  /**
   * Notifies the server that the specified hashed MAC address has been banned until the specified
   * instant.
   *
   ...
   * @param muteExpires The time at which the mute expires or {@code null} if the mute is
   *     indefinite.
   */
  void notifyUsernameMutingOfPlayer(String username, @Nullable Instant muteExpires);

  /**
   * Notifies the server that the specified hashed MAC address has been muted until the specified
   * instant.
   *
