...
public class ClientGame extends AbstractGame {
  ...
  public ClientGame(
    ...
    final IGameStepAdvancer gameStepAdvancer =
        (stepName, player) -> {
          ...
          if (gp == null) {
            throw new IllegalStateException(
                "Game player not found. Player:"
                    + player
                    + " on:"
                    + channelMessenger.getLocalNode());
          }
          gp.start(stepName);
        };
    remoteMessenger.registerRemote(
        gameStepAdvancer, getRemoteStepAdvancerName(channelMessenger.getLocalNode()));
  ...
  public void saveGame(final File f) {
    final IServerRemote server =
        (IServerRemote) remoteMessenger.getRemote(ServerGame.SERVER_REMOTE);
