 ...
 * hashmap -> attachment1, and this causes major problems for Java's deserializing.
 * When deserializing the attachments will not be resolved before their hashcode is taken, resulting in the wrong
 * hashcode and the attachment going in the wrong bucket, so that a .get(attachment1) will result in a null instead of
 * giving the key for attachment1. So just don't have maps of attachments, in an attachment. Thx, Veqryn.
 */
