...
class LobbyGamePanel extends JPanel {
  ...
  private void mouseOnGamesList(final MouseEvent e) {
    ...
    if (isAdmin()) {
      ...
      if (gameDescription.isBot()) {
        final Collection<Action> botAdminActions =
            getBotAdminGamesListContextActions(gameDescription);
  ...
  private static Action getSupportInfoHeadlessHostBotAction(final GameDescription gameDescription) {
    ...
    final String text =
        "Support Email for this bot is as follows. "
    ...
    return SwingAction.of(
        "Show Support Information/Email Of Headless Host Bot",
        e -> {
          final JTextPane textPane = new JTextPane();
          textPane.setEditable(false);
          textPane.setText(text);
          JOptionPane.showMessageDialog(
              null, textPane, "Bot Support Info", JOptionPane.INFORMATION_MESSAGE);
        });
  }
  ...
  protected void hostGame() {
    final ServerOptions options =
        new ServerOptions(
            ...
            messengers.getMessenger().getLocalNode().getName(),
            3300,
            true);
  ...
  private void mutePlayerInHeadlessHostBot() {
    ...
    final int result =
        JOptionPane.showConfirmDialog(
            null,
            "Are you sure you want to perform a remote mute player on this host?",
            "Remote Player Mute Headless Host Bot",
            JOptionPane.OK_CANCEL_OPTION);
    if (result != JOptionPane.OK_OPTION) {
      return;
    }
    final String playerToBeMuted =
        JOptionPane.showInputDialog(
            getTopLevelAncestor(),
            "Player Name To Be Muted?",
            "Player Name To Be Muted?",
    ...
    final String hashedPassword = hashPassword(password, salt);
    final String response =
        controller.mutePlayerHeadlessHostBot(
            lobbyWatcherNode, playerToBeMuted, min, hashedPassword, salt);
    JOptionPane.showMessageDialog(
        null,
        (response == null
  ...
  private void banPlayerInHeadlessHostBot() {
    ...
    final String response =
        controller.banPlayerHeadlessHostBot(
            lobbyWatcherNode, playerToBeBanned, hrs, hashedPassword, salt);
    JOptionPane.showMessageDialog(
        null,
        (response == null
  ...
  private void stopGameHeadlessHostBot() {
    ...
    JOptionPane.showMessageDialog(
        null,
        (response == null
            ? "Successfully attempted stop of current game on host"
            : "Failed: " + response));
  }
