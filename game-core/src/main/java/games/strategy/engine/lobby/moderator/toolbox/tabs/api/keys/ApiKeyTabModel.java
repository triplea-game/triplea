...
class ApiKeyTabModel {
  @VisibleForTesting static final String DELETE_BUTTON_TEXT = "Delete";
  ...
  List<List<String>> fetchTableData() {
    return toolboxApiKeyClient.getApiKeys().stream()
        .map(
            keyData ->
                Arrays.asList(
                    keyData.getPublicId(),
                    Optional.ofNullable(keyData.getLastUsed()).map(Instant::toString).orElse(""),
                    Optional.ofNullable(keyData.getLastUsedIp()).orElse(""),
                    DELETE_BUTTON_TEXT))
