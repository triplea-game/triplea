...
public final class ModeratorsTab implements Supplier<Component> {
  ...
  public ModeratorsTab(
      final JFrame parentFrame, final ToolboxModeratorManagementClient toolboxModeratorManagementClient) {
  ...
  private JTable buildTable() {
    ...
    if (moderatorsTabModel.isSuperMod()) {
      ...
      ButtonColumn.attachButtonColumn(table, 4, moderatorsTabActions.addSuperModAction(parentFrame));
    }
    return table;
  }
    ...
    DocumentListenerBuilder.attachDocumentListener(
        ...
        () -> new Thread(() -> {
          if (!checkUserNameIsInFlight) {
            ...
            SwingUtilities.invokeLater(() -> addButton.setEnabled(userNameRequested.length() >= USERNAME_MIN_LENGTH
