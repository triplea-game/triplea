...
/**
 * API key tab allows moderators to view their API keys, remove keys, and issue new temporary
 * keys that can be used to log in from additional computers.
 *
 * <pre>
 * +---------------------------------------------------------------------+
 * |   WINDOW LABEL HEADER                                               |
...
public final class ApiKeysTab implements Supplier<Component> {
  ...
  private static final String WINDOW_HEADER_LABEL = "<html>Use this window to remove keys that are no "
  ...
  public Component get() {
    return JPanelBuilder.builder()
        ...
        .addNorth(JLabelBuilder.builder()
            .text(WINDOW_HEADER_LABEL)
            .border(15)
            .build())
        .addCenter(SwingComponents.newJScrollPane(buildTable(parentFrame, apiKeyTabActions, apiKeyTabModel)))
