 ...
 * key stored in client settings to do API key validation with the server. If successful then we
 * show the toolbox window and the password is used to create a {@code ModeratorToolboxClient}. The
 * client will then send the API key and password on any further interactions with server.
 */
...
final class EnterApiKeyPasswordWindow {
  ...
  /**
   * Builds the enter api key password window. The window contains a field for entering the
   * moderators API-key password. The re-register button will clear the API-Key stored on the local
   * OS.
   *
   * <pre>
   * +---------------------------------------------------------+
  ...
  private static void showAction(final JFrame parent, final URI serverUri) {
    ...
    final JTextField apiKeyPasswordField =
        JTextFieldBuilder.builder()
            .columns(30)
            .actionListener(
                textField -> submitButton.setEnabled(!textField.getText().trim().isEmpty()))
            .build();
    ...
    submitButton.addActionListener(submitButtonAction(frame, apiKeyPasswordField, serverUri));
  }
  ...
  private static Runnable reRegisterButtonAction(final JFrame frame, final URI serverUri) {
    return () ->
  ...
  private static ActionListener submitButtonAction(
      final JFrame frame, final JTextField passwordField, final URI serverUri) {
    return e ->
        new BackgroundTaskRunner(frame)
            .awaitRunInBackground(
                "Verifying",
                () -> {
                  final ApiKeyPassword apiKeyPassword =
