...
/**
 * The purpose of this window is to give the user a field for entering their single-use
 * moderator key and to create a new password. The single-use password is issued to them
 * and is temporary. If the key is valid then the backend will salt a new key with the password
 * and returns a new key that the client side should then store in client settings.
 * From then on the moderator only needs to enter their password and their key is retrieved from
 * client settings.
 */
...
final class CreateNewApiKeyWindow {
  ...
  static void show(final JFrame parent, final URI serverUri) {
    ...
    final JTextField apiKeyField = JTextFieldBuilder.builder()
        .columns(30)
        .toolTip(API_KEY_FIELD_TOOL_TIP)
        .build();
    final JTextField passwordField = JTextFieldBuilder.builder()
        .columns(30)
        .toolTip(PASSWORD_FIELD_TOOLTIP)
        .build();

    DocumentListenerBuilder.attachDocumentListener(apiKeyField,
        enableSubmitButtonAction(apiKeyField, passwordField, submitButton));
    DocumentListenerBuilder.attachDocumentListener(passwordField,
        enableSubmitButtonAction(apiKeyField, passwordField, submitButton));
    ...
    submitButton
        .addActionListener(
            e -> new BackgroundTaskRunner(frame)
  ...
  /**
   * Creates a text field listener that will enable the submit button when the password
   * and api key fields have a minimum amount of text filled in.
   */
  private static Runnable enableSubmitButtonAction(
      final JTextComponent apiKeyField, final JTextComponent passwordField, final JButton submitButton) {
