...
/**
 * This class runs a set of local system checks, like access network, and create a temp file. Each
 * check is always run, and this class records the results of those checks.
 */
@Log
public final class LocalSystemChecker {
  ...
  private static SystemCheck defaultNetworkCheck() {
    return new SystemCheck(
        "Can connect to github.com (check network connection)",
        () -> {
          try {
  ...
  private static SystemCheck defaultFileSystemCheck() {
    return new SystemCheck(
        "Can create temporary files (check disk usage, file permissions)",
        () -> {
          try {
