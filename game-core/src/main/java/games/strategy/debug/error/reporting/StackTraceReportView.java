...
public interface StackTraceReportView {
  ...
  /**
   * Entry point for creating and display a 'stack trace report' window,
   * a window which provides a basic UI where users can enter an error
   * description before uploading an error report. The window is tied to a
   * stack trace or error that happened and would be uploaded as part of the report.
   * The user description is for developer benefit to get an opportunity for
   * more information around the circumstances of an error.
   */
  static void showWindow(
      final Component parentWindow,
      final ErrorUploadClient uploader,
      final LogRecord logRecord) {

    final StackTraceReportView window = new StackTraceReportSwingView(parentWindow);

    final StackTraceReportModel viewModel = StackTraceReportModel.builder()
        .view(window)
