...
class PreviewWindowTest {
  ...
  private static final UserErrorReport USER_ERROR_REPORT =
      UserErrorReport.builder().title(TITLE).description(DESCRIPTION).build();

  /**
   * Just check that we have rendered at least the user supplied info to the window in a text area,
   * and that the text area is read-only.
   */
  @Test
