...
public class BattleStepTWWTest {
  ...
  private static final GamePlayer BRITAIN =
      checkNotNull(GameDataTestUtil.britain(data));
  private static final GamePlayer AMERICAN =
      checkNotNull(GameDataTestUtil.usa(data));
  private static final GamePlayer GERMANS =
      checkNotNull(GameDataTestUtil.germany(data));
  private static final Territory LAND_NO_ATTACHMENTS = checkNotNull(GameDataTestUtil.territory("Alberta", data));
  ...
  void attackAaWithMultipleTargetGroups() {
    ...
    final StepUnits attackingUnits = new StepUnits(attackingOrderOfLoss, BRITAIN, defendingOrderOfLoss, GERMANS);
  ...
  void attackSuicideAa() {
    ...
    final StepUnits attackingUnits = new StepUnits(attackingOrderOfLoss, AMERICAN, defendingOrderOfLoss, GERMANS);
  ...
  void calculateComplexProbabilities() throws InterruptedException {
    ...
    final List<Unit> attackingOrderOfLoss = new ArrayList<>();
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishInfantry").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishAlpineInfantry").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishMarine").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishArtillery").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishHeavyArtillery").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishMobileArtillery").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishMech.Infantry").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishTank").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishHeavyTank").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishFighter").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishAdvancedFighter").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishTacticalBomber").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishAdvancedTacticalBomber").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishAntiAirGun").create(quantity, BRITAIN));
    attackingOrderOfLoss.addAll(data.getUnitTypeList().getUnitType("britishAntiTankGun").create(quantity, BRITAIN));

    final int diceSides = data.getDiceSides();
    final Map<Integer, Integer> diceGrouped = getDiceGrouped(attackingOrderOfLoss, List.of());

    final StepUnits attackingUnits = new StepUnits(attackingOrderOfLoss, BRITAIN, List.of(), GERMANS);
