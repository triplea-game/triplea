...
/**
 * Controller providing access for moderators to 'register' a single-use-key, a key given to them by
 * moderator admins, and in return the endpoint returns a new API key for the moderator to use from
 * then-on.
 */
@Path("")
@AllArgsConstructor
public class ApiKeyRegistrationController {
  ...
  /**
   * This method is used to consume a single use key and new password from user. If the single use
   * api-key is valid then the backend will generate a new key and salt it with the given password.
   * The backend will return the new to the user and store a hash of the salted password. If the
   * single-use key is not valid, a 400 is returned.
   */
  @POST
  ...
  public Response registerApiKey(@Context final HttpServletRequest request) {
    ...
    try {
      final String newApiKey =
          apiKeyRegistrationService.registerKey(
              request,
              request.getHeader(ToolboxHttpHeaders.API_KEY_HEADER),
              request.getHeader(ToolboxHttpHeaders.API_KEY_PASSWORD_HEADER));

      return Response.status(200).entity(RegisterApiKeyResult.newApiKeyResult(newApiKey)).build();
