...
/**
 * Provides endpoint for moderator maintenance actions and to support the moderators
 * toolbox 'moderators' tab. Actions include: adding moderators, removing moderators,
 * and promoting moderators to 'super-mod'. Some actions are only allowed for super-mods.
 */
@Path("")
...
public class ModeratorsController {
  ...
  public Response isSuperMod(@Context final HttpServletRequest request) {
    ...
    final Optional<Integer> result =
        apiKeyValidationService.lookupSuperModByApiKey(request);

    return Response
        .ok()
        .entity(result.isPresent())
        .build();
  }
