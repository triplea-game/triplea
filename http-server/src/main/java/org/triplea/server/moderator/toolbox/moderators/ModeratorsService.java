...
class ModeratorsService {
  ...
  List<ModeratorInfo> fetchModerators() {
    return moderatorsDao.getModerators()
        .stream()
        .map(userInfo -> ModeratorInfo.builder()
  ...
  void addModerator(final int moderatorIdRequesting, final String username) {
    final int userId = userLookupDao.lookupUserIdByName(username)
        .orElseThrow(() -> new IllegalArgumentException("Unable to find username: " + username));
  ...
  void removeMod(final int moderatorIdRequesting, final String moderatorNameToRemove) {
    final int userId = userLookupDao.lookupUserIdByName(moderatorNameToRemove)
  ...
  void addSuperMod(final int moderatorIdRequesting, final String username) {
    final int userId = userLookupDao.lookupUserIdByName(username)
        .orElseThrow(() -> new IllegalArgumentException("Failed to find user by name: " + username));
