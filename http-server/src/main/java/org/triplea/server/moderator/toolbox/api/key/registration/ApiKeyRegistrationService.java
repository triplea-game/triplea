...
/**
 * This service is used to consume a "single-use-key" and generate a new key. It is used by
 * moderators to obtain a new permanent key that is only known to them. The single-use-key is issues
 * to moderators and is rendered invalid after registration. The new key is salted with a password
 * provided by the moderator and stored as a hash. The unsalted new key is returned to the moderator
 * and is otherwise not stored anywhere else beyond the moderators OS.
 */
@Builder
@Log
public class ApiKeyRegistrationService {
  @Nonnull private final Function<String, String> singleKeyHasher;

  @Nonnull private final BiFunction<String, String, String> keyHasher;

  @Nonnull private final Supplier<String> newApiKeySupplier;
  ...
  @Nonnull private final ModeratorKeyRegistrationDao moderatorKeyRegistrationDao;

  @Nonnull private Predicate<String> apiKeyPasswordBlacklist;

  /**
   * Method to validate a given single-use-key, if valid then we generate a new API key, salt it
   * with password, store the salted password as a hashed value in DB and return to the front-end
   * the new API key.
   *
   * @throws ApiKeyLockOutException Thrown if the requestor has too many failed attempts, no
   *     validation was attempted, the request is rejected.
   * @throws IncorrectApiKeyException Thrown if the single-use key provided is not found in
   *     database.
  ...
  String registerKey(
    ...
    final int moderatorId =
        moderatorSingleUseKeyDao
            .lookupModeratorBySingleUseKey(hashedKey)
            .orElseThrow(
                () -> {
                  log.warning(
                      "API key registration failed, incorrect key. Attempted by host: "
