...
class MessagingService {
  ...
  @Nonnull private final BiConsumer<Session, ServerEventEnvelope> messageSender;
  /** Sends to all connected sessions. */
  @Nonnull private final BiConsumer<Session, ServerEventEnvelope> messageBroadcaster;
  ...
  private Optional<ClientEventEnvelope> deserializeFromJson(
      final Session session, final String message) {
    try {
      return Optional.of(ClientEventEnvelope.fromJson(message));
  ...
  private void handleClientEnvelope(
      final Session session, final ClientEventEnvelope clientEventEnvelope) {
    apiKeyDaoWrapper
  ...
  void handleError(final Session session, final Throwable throwable) {
    final ServerEventEnvelope error = chatEventProcessor.createErrorMessage();
