...
public class AbstractDropwizardTest {
  ...
  protected static <T> T newClient(final BiFunction<URI, ApiKeyPassword, T> clientFunction) {
    return clientFunction.apply(LOCALHOST, ApiKeyPassword.builder()
        .password("test")
        .apiKey("test")
        .build());
  }

  protected static <T> T newClientWithInvalidCreds(final BiFunction<URI, ApiKeyPassword, T> clientFunction) {
    return clientFunction.apply(LOCALHOST, ApiKeyPassword.builder()
