...
class LobbyChatIntegrationTest extends DropwizardTest {
  ...
  private static final ApiKey MODERATOR_API_KEY = TestData.API_KEY;
  ...
  private LobbyChatClient createModerator() {
    final LobbyChatClient moderator =
        ...
        new LobbyChatClient(DropwizardTest.localhost, MODERATOR_API_KEY);
  ...
  private LobbyChatClient createChatter() {
    final LobbyChatClient chatter = new LobbyChatClient(DropwizardTest.localhost, CHATTER_API_KEY);
  ...
  /** Runs through a chat sequence with two players exercising all chat functionality. */
  @Test
  ...
  private void moderatorConnects() throws Exception {
    moderator.connect();
  ...
  private void chatterConnects() throws Exception {
    chatter.connect();
  ...
  private void chatterChats() throws Exception {
    // chatter chats
  ...
  private void chatterSlapsMod() throws Exception {
    // chatter slaps mod
  ...
  private void chatterUpdatesStatus() throws Exception {
  ...
  private void chatterLeaves() throws Exception {
    // chatter disconnects
    chatter.close();
  ...
  private static <T> void waitForMessage(final Collection<T> messageBuffer) throws Exception {
  ...
  private static <T> void waitForMessage(final Collection<T> messageBuffer, final int minCount)
    ...
    while (messageBuffer.size() < minCount) {
      // wait for message to be processed
      Thread.sleep(10L);
      final long elapsed = System.currentTimeMillis() - start;
      if (elapsed > MESSAGE_TIMEOUT) {
        throw new RuntimeException("No messages received, timing out.");
      }
    }
  }
}
