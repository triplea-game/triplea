...
class UnitStackingLimitFilterTest extends AbstractDelegateTestCase {
  ...
  void testUnitAttachmentStackingLimit() {
    ...
    assertThat(callFilterUnits(fourTanks, british, uk)).isEqualTo(fourTanks);

    // the same four tanks are returned, even if we pass some existing units.
    assertThat(callFilterUnits(fourTanks, british, uk, infantry.create(2, british)))
        .isEqualTo(fourTanks);
    ...
    assertThat(countMatches(result, Matches.unitIsOfType(infantry))).isEqualTo(2);
    assertThat(countMatches(result, Matches.unitIsOfType(armour))).isEqualTo(2);
  }
  ...
  void testClassicAaStackingLimit() {
    ...
    assertThat(callFilterUnits(units, british, uk)).isEmpty();
  ...
  void testPlayerAttachmentStackingLimit() {
    ...
    assertThat(callFilterUnits(units, british, uk)).isEqualTo(units);
    ...
    assertThat(callFilterUnits(units, british, uk)).isEqualTo(expected);
  ...
  void testTerritoryEffectForbiddenUnits() {
    ...
    assertThat(callFilterUnits(units, british, westCanada)).isEmpty();
  }
}
