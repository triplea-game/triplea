...
public class UiTest implements ApplicationContext {
  ...
  public static void setup(final String[] args) {
    ...
    try {
      final Path toThisSource =
          Path.of(UiTest.class.getProtectionDomain().getCodeSource().getLocation().toURI());

      final Path gameApp =
          FileUtils.findFileInParentFolders(toThisSource, "game-app").orElseThrow();
      ...
      log.error(
          ...
          UiTest.class.getProtectionDomain().getCodeSource().getLocation(),
          e);
    }
  }
  ...
  /**
   * same as in HeadedGameRunner, but without <code>oesNothingAiProvider</code> and <code>
   * FlowFieldAiProvider</code>
   *
