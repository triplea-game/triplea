 ...
 * more than itself. Taking an objective moves the margin by two, once for the point won and once for
 * the point the defender lost.
 *
 * <p>The window is "since this player last acted", not "since this action started", and that
 * difference matters as soon as turns are chained into a game. Measuring only within an action would
 * pay a player for ground it took and never charge it for ground the opponent took back on the turn
 * in between, which trains a policy to attack and never cover. Anchoring on the player's previous
 * decision folds the opponent's whole turn into the next reward that player sees, and makes each
...
public final class StandardStrategicRewardFunction implements StrategicRewardFunction {
  ...
  public double reward(
    ...
    final int previous =
        marginWhenLastSeen.getOrDefault(player, margin(player, beforeScores));
