...
public class TechActivationDelegate extends BaseTripleADelegate {
  ...
  public void start() {
    ...
    // Activate techs
    final Collection<TechAdvance> advances = data.getTechDelegate().getAdvances(player);
    ...
    data.getTechDelegate().clearAdvances(player);
  ...
  private void shareTechnology() {
    ...
    for (final GamePlayer p : shareWith) {
      final Collection<TechAdvance> availableTechs =
          TechnologyDelegate.getAvailableTechs(p, getData().getTechnologyFrontier());
