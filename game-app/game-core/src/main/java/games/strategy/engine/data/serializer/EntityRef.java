...
/**
 * A text-format reference to a game entity: the identity of the thing pointed at, without its state.
 * This is the text-save analogue of {@link games.strategy.engine.data.GameObjectStreamData} (the
 * network path's name-only handle), generalized to also cover {@code Unit} (by UUID) and the entity
 * kinds that handle omitted.
 *
 ...
 * unit's UUID string. For {@code ATTACHMENT} it is the attachment name and {@code attachmentOwnerId}
 * carries the owning entity's reference (encoded as a nested ref).
 */
public record EntityRef(RefKind kind, String id, @Nullable EntityRef owner) {
  ...
  static EntityRef fromJson(final JsonObject json) {
    ...
    final EntityRef owner =
        json.has("owner") ? fromJson(json.getAsJsonObject("owner")) : null;
